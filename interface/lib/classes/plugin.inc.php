...
class plugin {
	...
	*/
	...
	private function loadPluginCache() {
		...
		$plugin_dirs = array();
		$plugin_dirs[] = RMNETDOV_LIB_PATH.FS_DIV.'plugins';
		
		if(is_dir(RMNETDOV_WEB_PATH)) {
			if($dh = opendir(RMNETDOV_WEB_PATH)) {
				while(($file = readdir($dh)) !== false) {
					if($file !== '.' && $file !== '..' && is_dir(RMNETDOV_WEB_PATH . FS_DIV . $file) && is_dir(RMNETDOV_WEB_PATH . FS_DIV . $file . FS_DIV . 'lib' . FS_DIV . 'plugin.d')) $plugin_dirs[] = RMNETDOV_WEB_PATH . FS_DIV . $file . FS_DIV . 'lib' . FS_DIV . 'plugin.d';
				}
				closedir($dh);
			}
		}
		...
		if(@is_array($_SESSION['s']['plugin_cache'][$event_name])) {
			foreach($_SESSION['s']['plugin_cache'][$event_name] as $rec) {
				...
				if($module_name != '') {
					...
					$plugin_file = RMNETDOV_WEB_PATH . FS_DIV . $module_name . FS_DIV . 'lib' . FS_DIV . 'plugin.d' . FS_DIV . $plugin_name . '.inc.php';
				} else {
					$plugin_file = RMNETDOV_LIB_PATH . FS_DIV . 'plugins' . FS_DIV . $plugin_name . '.inc.php';
				}
