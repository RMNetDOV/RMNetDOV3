...
class remoting_server extends remoting {
    ...
    */
	...
	public function server_get_app_version($session_id, $server_id = 0)
    {
		...
		if (!empty($session_id)) {
			if($server_id == 0) $rmnetdov_app_version = array('rmnetdov_app_version' => RMNETDOV_APP_VERSION);
			else {
				$rec = $app->db->queryOneRecord("SELECT data FROM monitor_data WHERE type = 'rmnetdov_info' AND server_id = ?", $server_id);
				$rec = unserialize($rec['data']);
				$rmnetdov_app_version = array('rmnetdov_app_version' => $rec['version']);
				unset($rec);
			}
			return $rmnetdov_app_version;
		} else {
			return false;
		}
	}
