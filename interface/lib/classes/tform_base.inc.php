...
class tform_base {
	...
	function loadFormDef($file, $module = '') {
		...
		include_once RMNETDOV_ROOT_PATH.'/lib/lang/'.$app->functions->check_language($_SESSION['s']['language']).'.lng';
				...
				if (!($field['formtype'] == 'PASSWORD' and $record[$key] == '')) {
					...
					if($action == "INSERT") {
						if($field['formtype'] == 'PASSWORD') {
							$sql_insert_key .= "`$key`, ";
							if ((isset($field['encryption']) && $field['encryption'] == 'CLEARTEXT') || (isset($record['_rmnetdov_pw_crypted']) && $record['_rmnetdov_pw_crypted'] == 1)) {
