...
class remoting_lib extends tform_base {
	...
	function encode($record, $tab = '', $dbencode = true) {
		...
		if(isset($record['_rmnetdov_pw_crypted'])) $new_record['_rmnetdov_pw_crypted'] = $record['_rmnetdov_pw_crypted']; // this one is not in form definitions!
	...
	function rmnetdov_sysuser_add($params, $insert_id){
		...
		$groups = $groupid;
		if(!isset($params['_rmnetdov_pw_crypted']) || $params['_rmnetdov_pw_crypted'] != 1) $password = $app->auth->crypt_password(stripslashes($password));
	...
	function rmnetdov_sysuser_update($params, $client_id){
		global $app;
		$username = $params["username"];
		...
		if(!isset($params['_rmnetdov_pw_crypted']) || $params['_rmnetdov_pw_crypted'] != 1) $password = $app->auth->crypt_password(stripslashes($clear_password));
	...
	function rmnetdov_sysuser_delete($client_id){
		global $app;
		$client_id = $app->functions->intval($client_id);
