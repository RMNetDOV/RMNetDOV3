...
class app {
	...
	public function uses($classes) {
		...
		if(is_array($cl)) {
			foreach($cl as $classname) {
				...
				if(!array_key_exists($classname, $this->_loaded_classes) && is_file(RMNETDOV_CLASS_PATH."/$classname.inc.php")) {
					include_once RMNETDOV_CLASS_PATH."/$classname.inc.php";
	...
	public function load($files) {
		...
		if(is_array($fl)) {
			foreach($fl as $file) {
				$file = trim($file);
				include_once RMNETDOV_CLASS_PATH."/$file.inc.php";
			}
		}
	}
	...
	public function lng($text) {
		...
		if($this->_language_inc != 1) {
			...
			if(isset($_SESSION['s']['module']['name'])) {
				...
				if(!file_exists(RMNETDOV_ROOT_PATH.'/'.$lng_file)) $lng_file = '/web/'.$_SESSION['s']['module']['name'].'/lib/lang/en.lng';
	...
	public function load_language_file($filename) {
		$filename = RMNETDOV_ROOT_PATH.'/'.$filename;
...
if(is_dir(RMNETDOV_CLASS_PATH.'/IDS') && !defined('REMOTE_API_CALL') && ($ids_security_config['ids_anon_enabled'] == 'yes' || $ids_security_config['ids_user_enabled'] == 'yes' || $ids_security_config['ids_admin_enabled'] == 'yes')) {
	$app->uses('ids');
	$app->ids->start();
}
