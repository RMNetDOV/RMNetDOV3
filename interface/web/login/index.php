...
function process_login_request(app $app, &$error, $conf, $module)
{
	...
	if ($user) {
		...
		if (is_file(RMNETDOV_WEB_PATH.'/'.$_SESSION['s']['user']['startmodule'].'/lib/module.conf.php')) {
			include_once $app->functions->check_include_path(RMNETDOV_WEB_PATH.'/'.$_SESSION['s']['user']['startmodule'].'/lib/module.conf.php');
			$menu_dir = RMNETDOV_WEB_PATH.'/'.$_SESSION['s']['user']['startmodule'].'/lib/menu.d';
			include_menu_dir_files($menu_dir);
			$_SESSION['s']['module'] = $module;
		}
		// check if the user theme is valid
		if ($_SESSION['s']['user']['theme'] != 'default') {
			$tmp_path = RMNETDOV_THEMES_PATH."/".$_SESSION['s']['user']['theme'];
		...
		$authlog_handle = fopen($conf['rmnetdov_log_dir'].'/auth.log', 'a');
