...
if(isset($_FILES['file']['name']) && is_uploaded_file($_FILES['file']['tmp_name'])) {
	...
	if($parts[0] == '---' && $parts[1] == 'RM-Net - DOV CP Language File') {
			...
			foreach($lines as $line) {
				...
				if(is_array($parts) && count($parts) > 0 && $parts[0] == '--') {
					...
					if($module_name == 'global') {
						$langfile_path = trim(RMNETDOV_LIB_PATH."/lang/".$selected_language.".lng");
					} else {
						$langfile_path = trim(RMNETDOV_WEB_PATH.'/'.$module_name.'/lib/lang/'.$file_name);
					}
