var RMNetDOV = {
	...
	setOption: function(key, value) {
		RMNetDOV.options[key] = value;
	},

	setOptions: function(opts) {
		$.extend(RMNetDOV.options, opts);
	},
	...
	registerHook: function(name, callback) {
		if(!RMNetDOV.registeredHooks[name]) RMNetDOV.registeredHooks[name] = new Array();
		var newindex = RMNetDOV.registeredHooks[name].length;
		RMNetDOV.registeredHooks[name][newindex] = callback;
	},

	callHook: function(name, params) {
		if(!RMNetDOV.registeredHooks[name]) return;
		for(var i = 0; i < RMNetDOV.registeredHooks[name].length; i++) {
			var callback = RMNetDOV.registeredHooks[name][i];
			callback(name, params);
		}
	},

	resetFormChanged: function() {
		RMNetDOV.pageFormChanged = false;
	},

	showLoadIndicator: function() {
		document.body.style.cursor = 'wait';

		if(RMNetDOV.options.useLoadIndicator == true) {
			RMNetDOV.requestsRunning += 1;

			if(RMNetDOV.requestsRunning < 2) {
				...
				RMNetDOV.indicatorCompleted = false;
				...
				indicator.css( {'left': atx, 'top': aty } ).fadeIn('fast', function() {
					...
					RMNetDOV.indicatorCompleted = true;
					if(RMNetDOV.requestsRunning < 1) $(this).fadeOut('fast', function() { $(this).hide();});
				});
			}
		}
	},
	...
	hideLoadIndicator: function() {
		...
		RMNetDOV.requestsRunning -= 1;
		if(RMNetDOV.requestsRunning < 1) {
			RMNetDOV.requestsRunning = 0; // just for the case...
			if(RMNetDOV.indicatorCompleted == true) $('#ajaxloader').fadeOut('fast', function() { $('#ajaxloader').hide(); } );
		}
	},

	onAfterSideNavLoaded: function() {
		if(RMNetDOV.options.useComboBox == true) {
	...
	onAfterContentLoad: function(url, data) {
		...
		RMNetDOV.callHook('onAfterContentLoad', {'url': url, 'data': data });
	},
	...
	submitForm: function(formname, target, confirmation) {
		...
		if(!confirmation || window.confirm(confirmation)) {
			var submitFormObj = $.ajax({
				...
				dataType: "html",
				beforeSend: function() {
					RMNetDOV.showLoadIndicator();
				},
				success: function(data, textStatus, jqXHR) {
					if(successMessage) alert(successMessage);
					if(jqXHR.responseText.indexOf('HEADER_REDIRECT:') > -1) {
						var parts = jqXHR.responseText.split(':');
						RMNetDOV.loadContent(parts[1]);
					} else if (jqXHR.responseText.indexOf('LOGIN_REDIRECT:') > -1) {
						// Go to the login page
						document.location.href = './index.php';
					} else {
						$('#pageContent').html(jqXHR.responseText);
						RMNetDOV.onAfterContentLoad(target, $('#'+formname).serialize());
						RMNetDOV.pageFormChanged = false;
					}
					clearTimeout(RMNetDOV.dataLogTimer);
					RMNetDOV.dataLogNotification();
					RMNetDOV.hideLoadIndicator();
				},
				error: function(jqXHR, textStatus, errorThrown) {
					RMNetDOV.hideLoadIndicator();
					var parts = jqXHR.responseText.split(':');
					RMNetDOV.reportError('Ajax Request was not successful. 111');
				}
			});
		}
	},
	...
	capp: function(module, redirect) {
		var submitFormObj = $.ajax({
			...
			error: function() {
				RMNetDOV.hideLoadIndicator();
				RMNetDOV.reportError('Ajax Request was not successful.'+module);
			}
		});
	},
		...
		var pageContentObject2 = $.ajax({
			...
			success: function(data, textStatus, jqXHR) {
				if(jqXHR.responseText.indexOf('HEADER_REDIRECT:') > -1) {
					...
					$('#pageContent').html(jqXHR.responseText);
					RMNetDOV.onAfterContentLoad(pagename, (params ? params : null));
					RMNetDOV.pageFormChanged = false;
				}
				clearTimeout(RMNetDOV.dataLogTimer); // clear running dataLogTimer
				RMNetDOV.dataLogNotification();
				RMNetDOV.hideLoadIndicator();
			},
			error: function() {
				RMNetDOV.hideLoadIndicator();
				RMNetDOV.reportError('Ajax Request was not successful. 113');
			}
		});
	},
		...
		if($('#refreshinterval').val() > 0) {
			var pageContentObject2 = $.ajax({
				...
				success: function(data, textStatus, jqXHR) {
					RMNetDOV.hideLoadIndicator();
					$('#pageContent').html(jqXHR.responseText);
					RMNetDOV.onAfterContentLoad(pagename, "refresh="+document.getElementById('refreshinterval').value);
					RMNetDOV.pageFormChanged = false;
				},
				error: function() {
					RMNetDOV.hideLoadIndicator();
					RMNetDOV.reportError('Ajax Request was not successful.'+pagename);
				}
			});
			setTimeout( "RMNetDOV.loadContentRefresh('"+pagename+"&refresh="+document.getElementById('refreshinterval').value+"')", document.getElementById('refreshinterval').value*1000*60 );
		}
	},
		...
		var pageContentObject = $.ajax({
			...
			success: function(data, textStatus, jqXHR) {
				if(jqXHR.responseText.indexOf('HEADER_REDIRECT:') > -1) {
					...
					$('#pageContent').html(jqXHR.responseText);
					RMNetDOV.onAfterContentLoad('dashboard/dashboard.php', "");
					RMNetDOV.pageFormChanged = false;
				}
				RMNetDOV.hideLoadIndicator();
			},
			error: function() {
				RMNetDOV.hideLoadIndicator();
				RMNetDOV.reportError('Ajax Request was not successful. 114');
			}
		});
	...
	loadMenus: function() {
		var sideNavObject = $.ajax({
			...
			success: function(data, textStatus, jqXHR) {
				...
				RMNetDOV.onAfterSideNavLoaded();
				RMNetDOV.loadPushyMenu();
			},
			error: function() {
				RMNetDOV.hideLoadIndicator();
				RMNetDOV.reportError('Ajax Request was not successful. 115');
			}
		});
		...
		var topNavObject = $.ajax({
			...
			error: function(o) {
				RMNetDOV.hideLoadIndicator();
				RMNetDOV.reportError('Ajax Request was not successful. 116');
			}
		});
	},

	changeTab: function(tab, target, force) {
		if(RMNetDOV.requestsRunning > 0) {
		...
		if(RMNetDOV.tabChangeDiscard == 'y' && !force) {
			if((idel.length < 1 || id) && (RMNetDOV.pageFormChanged == false || window.confirm(RMNetDOV.tabChangeDiscardTxt))) {
				var next_tab = tab;
				if(id) RMNetDOV.loadContent(target, {'next_tab': next_tab, 'id': id});
				else RMNetDOV.loadContent(target, {'next_tab': next_tab});
			} else {
				return false;
			}
		} else {
			if(id && RMNetDOV.tabChangeWarning == 'y' && RMNetDOV.pageFormChanged == true) {
				if(window.confirm(RMNetDOV.tabChangeWarningTxt)) {
					RMNetDOV.submitForm('pageForm', target);
				} else {
					var next_tab = tab;
					if(id) RMNetDOV.loadContent(target, {'next_tab': next_tab, 'id': id});
					else RMNetDOV.loadContent(target, {'next_tab': next_tab});
	...
	loadContentInto: function(elementid,pagename) {
		var pageContentObject2 = $.ajax({
			...
			error: function() {
				RMNetDOV.reportError('Ajax Request was not successful. 118');
			}
		});
	},
	...
	loadOptionInto: function(elementid,pagename,callback) {
		var pageContentObject2 = $.ajax({
			...
			error: function() {
				RMNetDOV.reportError('Ajax Request was not successful. 119');
			}
		});
	},
	...
	keepalive: function() {
		var pageContentObject3 = $.ajax({
			...
			dataType: "html",
			success: function(data, textStatus, jqXHR) {
				setTimeout( function() { RMNetDOV.keepalive(); }, 1000000 );
			},
			error: function() {
				RMNetDOV.reportError('Session expired. Please login again.');
			}
		});
	},
	dataLogNotification: function() {
		console.log(RMNetDOV.options);
	    var notificationContent = $.ajax({
			...
			success: function(data, textStatus, jqXHR) {
				...
				if(data['count'] > 0) {
					...
					RMNetDOV.dataLogTimer = setTimeout( function() { RMNetDOV.dataLogNotification(); }, 2000 );
				} else {
					$('.notification').css('display','none');
					$('.modal-body').html('');
					$('#datalogModal').modal('hide');
					RMNetDOV.dataLogTimer = setTimeout( function() { RMNetDOV.dataLogNotification(); }, 5000 );
				}
			},
			error: function() {
				RMNetDOV.reportError('Notification not loading, aborting.');
				$('.notification').css('display','none');
			}
		});
	},
	...
	addAdditionalTemplate: function(){
		...
		if(addTplId > 0) {
			...
			var delbtn = $('<a href="#"><span class="glyphicon glyphicon-remove-circle" aria-hidden="true"></span></a>').attr('class', 'btn btn-danger btn-xs').click(function(e) {
				e.preventDefault();
				RMNetDOV.delAdditionalTemplate($(this).parent().attr('rel'));
			});
			newVal[newVal.length] = 'n' + RMNetDOV.new_tpl_add_id + ':' + addTplId;
			$('<li>' + addTplText + '</li>').attr('rel', 'n' + RMNetDOV.new_tpl_add_id).append(delbtn).appendTo('#template_additional_list ul');
...
$(document).on('click', 'a[data-load-content],button[data-load-content]', function(e) {
	...
	RMNetDOV.loadContent(content_to_load);
});
...
$(document).on('click', 'a[data-submit-form],button[data-submit-form]', function(e) {
	...
	if($el.attr('data-form-upload') == 'true') RMNetDOV.submitUploadForm(form, act);
	else RMNetDOV.submitForm(form, act);
});
