...
/*
 * Check the RM-Net - DOV CP-Version (only for the admin)
*/
if($_SESSION["s"]["user"]["typ"] == 'admin') {
	if(!isset($_SESSION['s']['new_rmnetdov_version'])) {
		$new_version = @file_get_contents('https://www.rm-net.si/downloads/rmnetdov3_version.txt');
		$_SESSION['s']['new_rmnetdov_version'] = trim($new_version);
	}
	$v1 = RMNETDOV_APP_VERSION;
	$v2 = $_SESSION['s']['new_rmnetdov_version'];
...
/* Load the dashlets*/
$dashlet_list = array();
$handle = @opendir(RMNETDOV_WEB_PATH.'/dashboard/dashlets');
while ($file = @readdir($handle)) {
	if ($file != '.' && $file != '..' && !is_dir(RMNETDOV_WEB_PATH.'/dashboard/dashlets/'.$file)) {
		...
		if ($file_extension === 'php') { // only allow .php files 
			...
			include_once RMNETDOV_WEB_PATH.'/dashboard/dashlets/'.$file;
