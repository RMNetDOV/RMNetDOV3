...
Options:     yes/no
Description: Disables the shell user plugins in rmnetdov
...
Description: IDS score that triggers the log in /usr/local/rmnetdov/interface/temp/ids.log
             This log can be used to feed the whitelist.

			 Example:

			 cat /usr/local/rmnetdov/interface/temp/ids.log >> /usr/local/rmnetdov/security/ids.whitelist
			 rm -f /usr/local/rmnetdov/interface/temp/ids.log

			 If you want to use a custom whitelist, then store it as /usr/local/rmnetdov/security/ids.whitelist.custom
...
Description: Scan apache directives field for potentially malicious directives. This function uses the regex
             list from /usr/local/rmnetdov/security/apache_directives.blacklist file.
			 If you want to use a custom blacklist, then store it as /usr/local/rmnetdov/security/apache_directives.blacklist.custom
...
Options:     yes/no
Description: Warn by email when a new admin user in RM-Net - DOV CP has been added.
