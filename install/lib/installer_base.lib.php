...
class installer_base {
	...
	var $language = 'en';
	var $db;
	public $install_rmnetdov_interface = true;
	...
	public function update_acme() {
		$acme = explode("\n", shell_exec('which acme.sh /usr/local/rmnetdov/server/scripts/acme.sh /root/.acme.sh/acme.sh'));
		$acme = reset($acme);
		$val = 0;
	...
	*/
	...
	public function find_installed_apps() {
		...
		// RM-Net - DOV CP ships with vlogger, so it is always installed.
		$conf['vlogger']['installed'] = true;
		if(is_installed('crontab')) $conf['cron']['installed'] = true;

		if (($conf['apache']['installed'] && is_file($conf['apache']["vhost_conf_enabled_dir"]."/000-rmnetdov.vhost")) || ($conf['nginx']['installed'] && is_file($conf['nginx']["vhost_conf_enabled_dir"]."/000-rmnetdov.vhost"))) $this->rmnetdov_interface_installed = true;
	}
	...
	/** Create the database for RM-Net - DOV CP */
	...
	public function configure_database() {
		...
		if(count($db_tables) > 0) {
			...
			if($conf['mysql']['admin_password'] == '') {
				caselog("mysql --default-character-set=".escapeshellarg($conf['mysql']['charset'])." -h ".escapeshellarg($conf['mysql']['host'])." -u ".escapeshellarg($conf['mysql']['admin_user'])." -P ".escapeshellarg($conf['mysql']['port'])." ".escapeshellarg($conf['mysql']['database'])." < '".RMNETDOV_INSTALL_ROOT."/install/sql/rmnetdov3.sql' &> /dev/null",
					__FILE__, __LINE__, 'read in rmnetdov3.sql', 'could not read in rmnetdov3.sql');
			} else {
				caselog("mysql --default-character-set=".escapeshellarg($conf['mysql']['charset'])." -h ".escapeshellarg($conf['mysql']['host'])." -u ".escapeshellarg($conf['mysql']['admin_user'])." -p".escapeshellarg($conf['mysql']['admin_password'])." -P ".escapeshellarg($conf['mysql']['port'])." ".escapeshellarg($conf['mysql']['database'])." < '".RMNETDOV_INSTALL_ROOT."/install/sql/rmnetdov3.sql' &> /dev/null",
					__FILE__, __LINE__, 'read in rmnetdov3.sql', 'could not read in rmnetdov3.sql');
			}
			$db_tables = $this->db->getTables();
			if(count($db_tables) == 0) {
	...
	public function add_database_server_record() {
		...
		// Delete RM-Net - DOV CP user in the local database, in case that it exists
		$this->db->query("DROP USER ?@?", $conf['mysql']['rmnetdov_user'], $from_host);
		$this->db->query("DROP DATABASE IF EXISTS ?", $conf['mysql']['database']);

		//* Create the RM-Net - DOV CP database user and grant permissions in the local database
		$query = 'CREATE USER ?@? IDENTIFIED BY ?';
		if(!$this->db->query($query, $conf['mysql']['rmnetdov_user'], $from_host, $conf['mysql']['rmnetdov_password'])) {
			$this->error('Unable to create database user: '.$conf['mysql']['rmnetdov_user'].' Error: '.$this->db->errorMessage);
		}
		$query = 'GRANT SELECT, INSERT, UPDATE, DELETE ON ?? TO ?@?';
		if(!$this->db->query($query, $conf['mysql']['database'] . ".*", $conf['mysql']['rmnetdov_user'], $from_host)) {
			$this->error('Unable to grant databse permissions to user: '.$conf['mysql']['rmnetdov_user'].' Error: '.$this->db->errorMessage);
		}
		...
		// add correct administrative rights to IPSConfig user (SUPER is deprecated and unnecessarily powerful)
		 if ($this->db->getDatabaseType() == 'mysql' && $this->db->getDatabaseVersion(true) >= 8) {
			...
			if(!$this->db->query($query, $conf['mysql']['rmnetdov_user'], $from_host)) {
				$this->error('Unable to grant administrative permissions to user: '.$conf['mysql']['rmnetdov_user'].' Error: '.$this->db->errorMessage);
			}
		}
		...
		if($conf['mysql']['master_slave_setup'] == 'y') {
			...
			//* username for the rmnetdov user
			$conf['mysql']['master_rmnetdov_user'] = 'rmnetdovsrv'.$conf['server_id'];
	...
	public function grant_master_database_rights($verbose = false) {
		...
		*/
		...
		if($conf['mysql']['master_slave_setup'] == 'y') {
			...
			//* insert the rmnetdov user in the remote server
			$from_host = $conf['hostname'];
			$hosts[$from_host]['user'] = $conf['mysql']['master_rmnetdov_user'];
			$hosts[$from_host]['db'] = $conf['mysql']['master_database'];
			$hosts[$from_host]['pwd'] = $conf['mysql']['master_rmnetdov_password'];

			$host_ips = $this->get_host_ips();
			if(is_array($host_ips) && !empty($host_ips)) {
				foreach($host_ips as $ip) {
					$hosts[$ip]['user'] = $conf['mysql']['master_rmnetdov_user'];
					$hosts[$ip]['db'] = $conf['mysql']['master_database'];
					$hosts[$ip]['pwd'] = $conf['mysql']['master_rmnetdov_password'];
				}
			} else {
				$from_ip = gethostbyname($conf['hostname']);
				$hosts[$from_ip]['user'] = $conf['mysql']['master_rmnetdov_user'];
				$hosts[$from_ip]['db'] = $conf['mysql']['master_database'];
				$hosts[$from_ip]['pwd'] = $conf['mysql']['master_rmnetdov_password'];
			}
		} else{
			/*
			 ...
			 * host
			 */
			$query = "SELECT Host, User FROM mysql.user WHERE User like 'rmnetdovsrv%' ORDER BY User, Host";
		...
		if(count($hosts) > 0) {
			 ...
			 */
				...
				//* Create the RM-Net - DOV CP database user in the remote database
	...
	public function configure_jailkit() {
		...
		if (is_dir($config_dir)) {
			...
			if(is_file($conf['rmnetdov_install_dir'].'/server/conf-custom/install/'.$jk_init.'.master')) {
				copy($conf['rmnetdov_install_dir'].'/server/conf-custom/install/'.$jk_init.'.master', $config_dir.'/'.$jk_init);
			} else {
				copy('tpl/'.$jk_init.'.master', $config_dir.'/'.$jk_init);
			}
			if(is_file($conf['rmnetdov_install_dir'].'/server/conf-custom/install/'.$jk_chrootsh.'.master')) {
				copy($conf['rmnetdov_install_dir'].'/server/conf-custom/install/'.$jk_chrootsh.'.master', $config_dir.'/'.$jk_chrootsh);
			} else {
	...
	public function configure_mailman($status = 'insert') {
		...
		// load files
		$content = rfsel($conf['rmnetdov_install_dir'].'/server/conf-custom/install/mm_cfg.py.master', 'tpl/mm_cfg.py.master');
		...
		if(is_dir('/etc/mailman')) {
			if(is_file($conf['rmnetdov_install_dir'].'/server/conf-custom/install/mailman-virtual_to_transport.sh')) {
				copy($conf['rmnetdov_install_dir'].'/server/conf-custom/install/mailman-virtual_to_transport.sh', $full_file_name);
			} else {
	...
	public function configure_postfix($options = '') {
		...
		$content = rfsel($conf['rmnetdov_install_dir'].'/server/conf-custom/install/postfix-'.$filename.'.master', 'tpl/postfix-'.$filename.'.master');
		if($configure_lmtp) {
		...
		$postconf_tpl = rfsel($conf['rmnetdov_install_dir'].'/server/conf-custom/install/debian_postfix.conf.master', 'tpl/debian_postfix.conf.master');
		...
		if(file_exists($conf['rmnetdov_install_dir'].'/server/conf-custom/install/' . $configfile . '.master')) {
			$content = file_get_contents($conf['rmnetdov_install_dir'].'/server/conf-custom/install/'.$configfile.'.master');
			$content = strtr($content, $postconf_placeholders);
			$postconf_commands = array_merge($postconf_commands, array_filter(explode("\n", $content)));
		}
	...
	public function configure_pam() {
		global $conf;
		$pam = $conf['pam'];
		//* configure pam for SMTP authentication agains the rmnetdov database
	...
	public function configure_dovecot() {
		...
		if(version_compare($dovecot_version,1, '<=')) {	//* Dovecot 1.x
			if(is_file($conf['rmnetdov_install_dir'].'/server/conf-custom/install/debian_dovecot.conf.master')) {
				copy($conf['rmnetdov_install_dir'].'/server/conf-custom/install/debian_dovecot.conf.master', $config_dir.'/'.$configfile);
			} else {
				copy('tpl/debian_dovecot.conf.master', $config_dir.'/'.$configfile);
			}
		} else {	//* Dovecot 2.x
			if(is_file($conf['rmnetdov_install_dir'].'/server/conf-custom/install/debian_dovecot2.conf.master')) {
				copy($conf['rmnetdov_install_dir'].'/server/conf-custom/install/debian_dovecot2.conf.master', $config_dir.'/'.$configfile);
			} else {
				copy('tpl/debian_dovecot2.conf.master', $config_dir.'/'.$configfile);
			}
			// Copy custom config file
			if(is_file($conf['rmnetdov_install_dir'].'/server/conf-custom/install/dovecot_custom.conf.master')) {
				if(!@is_dir($config_dir . '/conf.d')) {
					mkdir($config_dir . '/conf.d');
				}
				copy($conf['rmnetdov_install_dir'].'/server/conf-custom/install/dovecot_custom.conf.master', $config_dir.'/conf.d/99-rmnetdov-custom-config.conf');
			}
				...
				if(!file_exists('/etc/dovecot/dh.pem')) {
					...
					if(file_exists('/var/lib/dovecot/ssl-parameters.dat')) {
						...
						/*
						   ...
						   a 4096 bit file, create it manually before you install RM-Net - DOV CP
						*/
		...
		$content = rfsel($conf['rmnetdov_install_dir'].'/server/conf-custom/install/debian_dovecot-sql.conf.master', 'tpl/debian_dovecot-sql.conf.master');
		$content = str_replace('{mysql_server_rmnetdov_user}', $conf['mysql']['rmnetdov_user'], $content);
		$content = str_replace('{mysql_server_rmnetdov_password}', $conf['mysql']['rmnetdov_password'], $content);
		$content = str_replace('{mysql_server_database}', $conf['mysql']['database'], $content);
		$content = str_replace('{mysql_server_host}', $conf['mysql']['host'], $content);
	...
	public function configure_rspamd() {
		...
		foreach ($local_d as $f) {
			...
			if (file_exists($conf['rmnetdov_install_dir']."/server/conf-custom/install/rspamd_${f}.master")) {
				$tpl->newTemplate($conf['rmnetdov_install_dir']."/server/conf-custom/install/rspamd_${f}.master");
			} else {
		...
		foreach ($local_d as $f) {
			if(file_exists($conf['rmnetdov_install_dir']."/server/conf-custom/install/rspamd_${f}.master")) {
				exec('cp '.$conf['rmnetdov_install_dir']."/server/conf-custom/install/rspamd_${f}.master /etc/rspamd/local.d/${f}");
			} else {
		...
		foreach ($override_d as $f) {
			if(file_exists($conf['rmnetdov_install_dir']."/server/conf-custom/install/rspamd_${f}.master")) {
				exec('cp '.$conf['rmnetdov_install_dir']."/server/conf-custom/install/rspamd_${f}.master /etc/rspamd/override.d/${f}");
			} else {
		...
		foreach ($maps_d as $f) {
			if(file_exists($conf['rmnetdov_install_dir']."/server/conf-custom/install/rspamd_${f}.master")) {
				exec('cp '.$conf['rmnetdov_install_dir']."/server/conf-custom/install/rspamd_${f}.master /etc/rspamd/local.d/maps.d/${f}");
			} else {
		...
		$tpl = new tpl();
		if (file_exists($conf['rmnetdov_install_dir']."/server/conf-custom/install/rspamd_worker-controller.inc.master")) {
			$tpl->newTemplate($conf['rmnetdov_install_dir']."/server/conf-custom/install/rspamd_worker-controller.inc.master");
		} else {
	...
	public function configure_pureftpd() {
		...
		//* configure pure-ftpd for MySQL authentication against the rmnetdov database
		...
		$content = rfsel($conf['rmnetdov_install_dir'].'/server/conf-custom/install/pureftpd_mysql.conf.master', 'tpl/pureftpd_mysql.conf.master');
		$content = str_replace('{mysql_server_rmnetdov_user}', $conf['mysql']['rmnetdov_user'], $content);
		$content = str_replace('{mysql_server_rmnetdov_password}', $conf['mysql']['rmnetdov_password'], $content);
		$content = str_replace('{mysql_server_database}', $conf['mysql']['database'], $content);
		$content = str_replace('{mysql_server_ip}', $conf['mysql']['ip'], $content);
	...
	public function configure_powerdns() {
		...
		//* Create the RM-Net - DOV CP database user in the local database
		$query = "GRANT ALL ON ??.* TO ?@?";
		if(!$this->db->query($query, $conf['powerdns']['database'], $conf['mysql']['rmnetdov_user'], 'localhost')) {
			$this->error('Unable to create user for powerdns database Error: '.$this->db->errorMessage);
		}

		//* load the powerdns databse dump
		if($conf['mysql']['admin_password'] == '') {
			caselog("mysql --default-character-set=".$conf['mysql']['charset']." -h '".$conf['mysql']['host']."' -u '".$conf['mysql']['admin_user']."' --force '".$conf['powerdns']['database']."' < '".RMNETDOV_INSTALL_ROOT."/install/sql/powerdns.sql' &> /dev/null",
				__FILE__, __LINE__, 'read in rmnetdov3.sql', 'could not read in powerdns.sql');
		} else {
			caselog("mysql --default-character-set=".$conf['mysql']['charset']." -h '".$conf['mysql']['host']."' -u '".$conf['mysql']['admin_user']."' -p'".$conf['mysql']['admin_password']."' --force '".$conf['powerdns']['database']."' < '".RMNETDOV_INSTALL_ROOT."/install/sql/powerdns.sql' &> /dev/null",
				__FILE__, __LINE__, 'read in rmnetdov3.sql', 'could not read in powerdns.sql');
		}
	...
	public function process_bind_file($configfile, $target='/', $absolute=false) {
		...
		else $full_file_name = $conf['rmnetdov_install_dir'].$target.$configfile;
		...
		$content = str_replace('{mysql_server_rmnetdov_user}', $conf['mysql']['rmnetdov_user'], $content);
		$content = str_replace('{mysql_server_rmnetdov_password}', $conf['mysql']['rmnetdov_password'], $content);
		$content = str_replace('{mysql_server_rmnetdov_database}', $conf['mysql']['database'], $content);
		$content = str_replace('{mysql_server_ip}', $conf['mysql']['ip'], $content);
		$content = str_replace('{rmnetdov_install_dir}', $conf['rmnetdov_install_dir'], $content);
		$content = str_replace('{dnssec_conffile}', $conf['rmnetdov_install_dir'].'/server/scripts/dnssec-config.sh', $content);
		wf($full_file_name, $content);
	}
	...
	public function configure_apache() {
		...
		if(!@is_dir($conf['rmnetdov_log_dir'].'/httpd')) mkdir($conf['rmnetdov_log_dir'].'/httpd', 0755, true);
		...
		if(is_file('/etc/apache2/ports.conf')) {
			...
			// Comment out the namevirtualhost lines, as they were added by rmnetdov in rmnetdov.conf file again
		...
		//* Copy the RM-Net - DOV CP configuration include
		$vhost_conf_dir = $conf['apache']['vhost_conf_dir'];
		$vhost_conf_enabled_dir = $conf['apache']['vhost_conf_enabled_dir'];

		$tpl = new tpl('apache_rmnetdov.conf.master');
		...
		wf($vhost_conf_dir.'/rmnetdov.conf', $tpl->grab());
		unset($tpl);

		if(!@is_link($vhost_conf_enabled_dir.'/000-rmnetdov.conf')) {
			symlink($vhost_conf_dir.'/rmnetdov.conf', $vhost_conf_enabled_dir.'/000-rmnetdov.conf');
		}
	...
	public function configure_bastille_firewall() {
		...
		if(is_file($conf['rmnetdov_install_dir'].'/server/conf-custom/install/bastille-firewall.cfg.master')) {
			caselog('cp -f ' . $conf['rmnetdov_install_dir'].'/server/conf-custom/install/bastille-firewall.cfg.master /etc/Bastille/bastille-firewall.cfg', __FILE__, __LINE__);
		} else {
	...
	public function configure_vlogger() {
		...
		$content = rfsel($conf['rmnetdov_install_dir'].'/server/conf-custom/install/'.$configfile.'.master', 'tpl/'.$configfile.'.master');
		if($conf['mysql']['master_slave_setup'] == 'y') {
			$content = str_replace('{mysql_server_rmnetdov_user}', $conf['mysql']['master_rmnetdov_user'], $content);
			$content = str_replace('{mysql_server_rmnetdov_password}', $conf['mysql']['master_rmnetdov_password'], $content);
	...
	public function configure_apps_vhost() {
		...
		//* Create the rmnetdov apps vhost user and group
		if($conf['apache']['installed'] == true){
			...
			if(is_file($conf['rmnetdov_install_dir'].'/interface/ssl/rmnetdovserver.crt') && is_file($conf['rmnetdov_install_dir'].'/interface/ssl/rmnetdovserver.key')) {
				$tpl->setVar('ssl_comment','');
			} else {
				$tpl->setVar('ssl_comment','#');
			}
			if(is_file($install_dir.'/interface/ssl/rmnetdovserver.crt') && is_file($install_dir.'/interface/ssl/rmnetdovserver.key') && is_file($install_dir.'/interface/ssl/rmnetdovserver.bundle')) {
			...
			//copy('tpl/apache_rmnetdov.vhost.master', "$vhost_conf_dir/rmnetdov.vhost");
			...
			if(!is_file($conf['web']['website_basedir'].'/php-fcgi-scripts/apps/.php-fcgi-starter')) {
				$content = rfsel($conf['rmnetdov_install_dir'].'/server/conf-custom/install/apache_apps_fcgi_starter.master', 'tpl/apache_apps_fcgi_starter.master');
		...
		if($conf['nginx']['installed'] == true){
			...
			// Dont just copy over the virtualhost template but add some custom settings
			$content = rfsel($conf['rmnetdov_install_dir'].'/server/conf-custom/install/nginx_apps.vhost.master', 'tpl/nginx_apps.vhost.master');
			...
			$content = rfsel($conf['rmnetdov_install_dir'].'/server/conf-custom/install/apps_php_fpm_pool.conf.master', 'tpl/apps_php_fpm_pool.conf.master');
			...
			//copy('tpl/nginx_rmnetdov.vhost.master', "$vhost_conf_dir/rmnetdov.vhost");
			//* and create the symlink
			if(@is_link($vhost_conf_enabled_dir.'/apps.vhost')) unlink($vhost_conf_enabled_dir.'/apps.vhost');
	...
	public function make_acme_vhost($server = 'apache') {
		...
		$acme_dir = $conf['rmnetdov_install_dir'] . '/interface/acme';

		//* Create the RM-Net - DOV CP installation directory
		if(!@is_dir($acme_dir)) {
	...
	public function make_rmnetdov_ssl_cert() {
		global $conf, $autoinstall;
		...
		//* Define and check RM-Net - DOV CP SSL folder */
		$ssl_dir = $conf['rmnetdov_install_dir'].'/interface/ssl';
		if(!@is_dir($ssl_dir)) {
			mkdir($ssl_dir, 0755, true);
		}

		$ssl_crt_file = $ssl_dir.'/rmnetdovserver.crt';
		$ssl_csr_file = $ssl_dir.'/rmnetdovserver.csr';
		$ssl_key_file = $ssl_dir.'/rmnetdovserver.key';
		$ssl_pem_file = $ssl_dir.'/rmnetdovserver.pem';
		...
		$acme_cert_dir = '/usr/local/rmnetdov/server/scripts/' . $hostname;
		...
		if(!is_dir($conf['rmnetdov_log_dir'])) {
			mkdir($conf['rmnetdov_log_dir'], 0755, true);
		}
		$acme_log = $conf['rmnetdov_log_dir'] . '/acme.log';
		...
		// Get subject and issuer of rmnetdovserver.crt to check if it is self-signed cert
		...
		if ((@file_exists($ssl_crt_file) && ($crt_subject == $crt_issuer)) || (!@is_dir($acme_cert_dir) || !@file_exists($check_acme_file) || !@file_exists($ssl_crt_file) || md5_file($check_acme_file) != md5_file($ssl_crt_file)) && $ip_address_match == true) {
			...
			if(file_exists(RMNETDOV_INSTALL_ROOT . '/server/scripts/letsencrypt_pre_hook.sh') && !file_exists('/usr/local/bin/letsencrypt_pre_hook.sh')) {
				if(is_link('/usr/local/bin/letsencrypt_pre_hook.sh')) {
					unlink('/usr/local/bin/letsencrypt_pre_hook.sh');
				}
				symlink(RMNETDOV_INSTALL_ROOT . '/server/scripts/letsencrypt_pre_hook.sh', '/usr/local/bin/letsencrypt_pre_hook.sh');
				chown('/usr/local/bin/letsencrypt_pre_hook.sh', 'root');
				chmod('/usr/local/bin/letsencrypt_pre_hook.sh', 0700);
			}
			if(file_exists(RMNETDOV_INSTALL_ROOT . '/server/scripts/letsencrypt_post_hook.sh') && !file_exists('/usr/local/bin/letsencrypt_post_hook.sh')) {
				if(is_link('/usr/local/bin/letsencrypt_post_hook.sh')) {
					unlink('/usr/local/bin/letsencrypt_post_hook.sh');
				}
				symlink(RMNETDOV_INSTALL_ROOT . '/server/scripts/letsencrypt_post_hook.sh', '/usr/local/bin/letsencrypt_post_hook.sh');
				chown('/usr/local/bin/letsencrypt_post_hook.sh', 'root');
				chmod('/usr/local/bin/letsencrypt_post_hook.sh', 0700);
			}
			if(file_exists(RMNETDOV_INSTALL_ROOT . '/server/scripts/letsencrypt_renew_hook.sh') && !file_exists('/usr/local/bin/letsencrypt_renew_hook.sh')) {
				if(is_link('/usr/local/bin/letsencrypt_renew_hook.sh')) {
					unlink('/usr/local/bin/letsencrypt_renew_hook.sh');
				}
				symlink(RMNETDOV_INSTALL_ROOT . '/server/scripts/letsencrypt_renew_hook.sh', '/usr/local/bin/letsencrypt_renew_hook.sh');
			...
			if($conf['nginx']['installed'] == true) {
				...
				if($this->is_update == false && @is_link($vhost_conf_enabled_dir.'/000-rmnetdov.conf')) {
					$restore_conf_symlink = true;
					unlink($vhost_conf_enabled_dir.'/000-rmnetdov.conf');
				}
				$server = 'apache';
			}
			...
			// Backup existing rmnetdovserver ssl files
			//
			...
			if (is_executable($acme)) {
				...
				# acme.sh does not set umask, resulting in incorrect permissions (rmnetdov issue #6015)
				...
				if($conf['nginx']['installed'] == true || $conf['apache']['installed'] == true) {
					exec("$acme --issue --log $acme_log -w /usr/local/rmnetdov/interface/acme -d " . escapeshellarg($hostname) . " $renew_hook", $out, $ret);
				}
			...
			// Else, we attempt to use the official LE certbot client certbot
				...
				if(is_executable($le_client)) {
					...
					if($conf['nginx']['installed'] == true || $conf['apache']['installed'] == true) {
						exec("$le_client $certonly $acme_version --authenticator webroot --webroot-path /usr/local/rmnetdov/interface/acme --email " . escapeshellarg('postmaster@' . $hostname) . " -d " . escapeshellarg($hostname) . " $renew_hook", $out, $ret);
					}
					// Else, it is not webserver, so we use standalone
					else {
		...
		// Build rmnetdovserver.pem file and chmod it
		if(file_exists($ssl_key_file)) {
			...
			if ($conf['postfix']['installed'] == true && strtolower($this->simple_query('Symlink RM-Net - DOV CP SSL certs to Postfix?', array('y', 'n'), 'y','rmnetdov_postfix_ssl_symlink')) == 'y') {
				...
				// Create symlink to RM-Net - DOV CP SSL files
				symlink($ssl_crt_file, $smtpd_crt);
				symlink($ssl_key_file, $smtpd_key);
			}

			// Extend LE SSL certs to pureftpd
			if ($conf['pureftpd']['installed'] == true && strtolower($this->simple_query('Symlink RM-Net - DOV CP SSL certs to Pure-FTPd? Creating dhparam file may take some time.', array('y', 'n'), 'y','rmnetdov_pureftpd_ssl_symlink')) == 'y') {
	...
	public function install_rmnetdov() {
		global $conf;

		$install_dir = $conf['rmnetdov_install_dir'];
		...
		//* Create a RM-Net - DOV CP user and group
		$command = 'groupadd rmnetdov';
		if(!is_group('rmnetdov')) caselog($command.' &> /dev/null 2> /dev/null', __FILE__, __LINE__, "EXECUTED: $command", "Failed to execute the command $command");

		$command = 'useradd -g rmnetdov -d '.$install_dir.' rmnetdov';
		if(!is_user('rmnetdov')) caselog($command.' &> /dev/null 2> /dev/null', __FILE__, __LINE__, "EXECUTED: $command", "Failed to execute the command $command");

		//* copy the RM-Net - DOV CP interface part
		$command = 'cp -rf ../interface '.$install_dir;
		caselog($command.' &> /dev/null', __FILE__, __LINE__, "EXECUTED: $command", "Failed to execute the command $command");

		//* copy the RM-Net - DOV CP server part
		$command = 'cp -rf ../server '.$install_dir;
		caselog($command.' &> /dev/null', __FILE__, __LINE__, "EXECUTED: $command", "Failed to execute the command $command");

		//* Make a backup of the security settings
		if(is_file('/usr/local/rmnetdov/security/security_settings.ini')) copy('/usr/local/rmnetdov/security/security_settings.ini','/usr/local/rmnetdov/security/security_settings.ini~');

		//* copy the RM-Net - DOV CP security part
		...
		//* Create a symlink, so RM-Net - DOV CP is accessible via web
		// Replaced by a separate vhost definition for port 8080
		// $command = "ln -s $install_dir/interface/web/ /var/www/rmnetdov";
		// caselog($command.' &> /dev/null', __FILE__, __LINE__, "EXECUTED: $command", "Failed to execute the command $command");

		//* Create the config file for RM-Net - DOV CP interface
		...
		$content = str_replace('{mysql_master_server_rmnetdov_user}', $conf['mysql']['master_rmnetdov_user'], $content);
		$content = str_replace('{mysql_master_server_rmnetdov_password}', $conf['mysql']['master_rmnetdov_password'], $content);
		$content = str_replace('{mysql_master_server_database}', $conf['mysql']['master_database'], $content);
		$content = str_replace('{mysql_master_server_host}', $conf['mysql']['master_host'], $content);
		$content = str_replace('{mysql_master_server_port}', $conf['mysql']['master_port'], $content);

		$content = str_replace('{server_id}', $conf['server_id'], $content);
		$content = str_replace('{rmnetdov_log_priority}', $conf['rmnetdov_log_priority'], $content);
		...
		//* Create the config file for RM-Net - DOV CP server
		$configfile = 'config.inc.php';
		...
		//* chown the interface files to the rmnetdov user and group
		$command = 'chown -R rmnetdov:rmnetdov '.$install_dir.'/interface';
		caselog($command.' &> /dev/null', __FILE__, __LINE__, "EXECUTED: $command", "Failed to execute the command $command");
		...
		//* chown the security directory and security_settings.ini to root:rmnetdov
		$command = 'chown root:rmnetdov '.$install_dir.'/security/security_settings.ini';
		caselog($command.' &> /dev/null', __FILE__, __LINE__, "EXECUTED: $command", "Failed to execute the command $command");
		$command = 'chown root:rmnetdov '.$install_dir.'/security';
		caselog($command.' &> /dev/null', __FILE__, __LINE__, "EXECUTED: $command", "Failed to execute the command $command");
		$command = 'chown root:rmnetdov '.$install_dir.'/security/ids.whitelist';
		caselog($command.' &> /dev/null', __FILE__, __LINE__, "EXECUTED: $command", "Failed to execute the command $command");
		$command = 'chown root:rmnetdov '.$install_dir.'/security/ids.htmlfield';
		caselog($command.' &> /dev/null', __FILE__, __LINE__, "EXECUTED: $command", "Failed to execute the command $command");
		$command = 'chown root:rmnetdov '.$install_dir.'/security/apache_directives.blacklist';
		caselog($command.' &> /dev/null', __FILE__, __LINE__, "EXECUTED: $command", "Failed to execute the command $command");
		$command = 'chown root:rmnetdov '.$install_dir.'/security/nginx_directives.blacklist';
		caselog($command.' &> /dev/null', __FILE__, __LINE__, "EXECUTED: $command", "Failed to execute the command $command");
		...
		//* Make sure thet the interface config file is readable by user rmnetdov only
		chmod($install_dir.'/interface/lib/config.inc.php', 0600);
		chown($install_dir.'/interface/lib/config.inc.php', 'rmnetdov');
		chgrp($install_dir.'/interface/lib/config.inc.php', 'rmnetdov');
		...
		if(is_dir($install_dir.'/interface/invoices')) {
			...
			exec('chown -R rmnetdov:rmnetdov '.escapeshellarg($install_dir.'/interface/invoices'));
		}

		exec('chown -R root:root /usr/local/rmnetdov/interface/ssl');

		// TODO: FIXME: add the www-data user to the rmnetdov group. This is just for testing
		// and must be fixed as this will allow the apache user to read the rmnetdov files.
		// Later this must run as own apache server or via suexec!
		if($conf['apache']['installed'] == true){
			$command = 'adduser '.$conf['apache']['user'].' rmnetdov';
			caselog($command.' &> /dev/null', __FILE__, __LINE__, "EXECUTED: $command", "Failed to execute the command $command");
			if(is_group('ispapps')){
		...
		if($conf['nginx']['installed'] == true){
			$command = 'adduser '.$conf['nginx']['user'].' rmnetdov';
			caselog($command.' &> /dev/null', __FILE__, __LINE__, "EXECUTED: $command", "Failed to execute the command $command");
			if(is_group('ispapps')){
		...
		if ($this->install_rmnetdov_interface == true && isset($conf['interface_password']) && $conf['interface_password']!='admin') {
			$sql = "UPDATE sys_user SET passwort = ? WHERE username = 'admin';";
			$this->db->query($sql, $this->crypt_password($conf['interface_password']));
		}

		if($conf['apache']['installed'] == true && $this->install_rmnetdov_interface == true){
			//* Copy the RM-Net - DOV CP vhost for the controlpanel
			$vhost_conf_dir = $conf['apache']['vhost_conf_dir'];
			$vhost_conf_enabled_dir = $conf['apache']['vhost_conf_enabled_dir'];

			// Dont just copy over the virtualhost template but add some custom settings
			$tpl = new tpl('apache_rmnetdov.vhost.master');
			...
			if(is_file($install_dir.'/interface/ssl/rmnetdovserver.crt') && is_file($install_dir.'/interface/ssl/rmnetdovserver.key')) {
				$tpl->setVar('ssl_comment','');
			} else {
				$tpl->setVar('ssl_comment','#');
			}
			...
			wf($vhost_conf_dir.'/rmnetdov.vhost', $tpl->grab());

			//* and create the symlink
			if($this->is_update == false) {
				if(@is_link($vhost_conf_enabled_dir.'/rmnetdov.vhost')) unlink($vhost_conf_enabled_dir.'/rmnetdov.vhost');
				if(!@is_link($vhost_conf_enabled_dir.'/000-rmnetdov.vhost')) {
					symlink($vhost_conf_dir.'/rmnetdov.vhost', $vhost_conf_enabled_dir.'/000-rmnetdov.vhost');
				}
			}
			//if(!is_file('/var/www/php-fcgi-scripts/rmnetdov/.php-fcgi-starter')) {
			$content = rfsel($conf['rmnetdov_install_dir'].'/server/conf-custom/install/apache_rmnetdov_fcgi_starter.master', 'tpl/apache_rmnetdov_fcgi_starter.master');
			$content = str_replace('{fastcgi_bin}', $conf['fastcgi']['fastcgi_bin'], $content);
			$content = str_replace('{fastcgi_phpini_path}', $conf['fastcgi']['fastcgi_phpini_path'], $content);
			@mkdir('/var/www/php-fcgi-scripts/rmnetdov', 0755, true);
			$this->set_immutable('/var/www/php-fcgi-scripts/rmnetdov/.php-fcgi-starter', false);
			wf('/var/www/php-fcgi-scripts/rmnetdov/.php-fcgi-starter', $content);
			exec('chmod +x /var/www/php-fcgi-scripts/rmnetdov/.php-fcgi-starter');
			@symlink($install_dir.'/interface/web', '/var/www/rmnetdov');
			exec('chown -R rmnetdov:rmnetdov /var/www/php-fcgi-scripts/rmnetdov');
			$this->set_immutable('/var/www/php-fcgi-scripts/rmnetdov/.php-fcgi-starter', true);
			//}
		}

		if($conf['nginx']['installed'] == true && $this->install_rmnetdov_interface == true){
			//* Copy the RM-Net - DOV CP vhost for the controlpanel
			$vhost_conf_dir = $conf['nginx']['vhost_conf_dir'];
			$vhost_conf_enabled_dir = $conf['nginx']['vhost_conf_enabled_dir'];

			// Dont just copy over the virtualhost template but add some custom settings
			$content = rfsel($conf['rmnetdov_install_dir'].'/server/conf-custom/install/nginx_rmnetdov.vhost.master', 'tpl/nginx_rmnetdov.vhost.master');
			...
			if(substr($socket_dir, -1) != '/') $socket_dir .= '/';
			if(!is_dir($socket_dir)) exec('mkdir -p '.$socket_dir);
			$fpm_socket = $socket_dir.'rmnetdov.sock';

			//$content = str_replace('{fpm_port}', $conf['nginx']['php_fpm_start_port'], $content);
			$content = str_replace('{fpm_socket}', $fpm_socket, $content);

			wf($vhost_conf_dir.'/rmnetdov.vhost', $content);

			unset($content);

			// PHP-FPM
			// Dont just copy over the php-fpm pool template but add some custom settings
			$content = rfsel($conf['rmnetdov_install_dir'].'/server/conf-custom/install/php_fpm_pool.conf.master', 'tpl/php_fpm_pool.conf.master');
			$content = str_replace('{fpm_pool}', 'rmnetdov', $content);
			//$content = str_replace('{fpm_port}', $conf['nginx']['php_fpm_start_port'], $content);
			$content = str_replace('{fpm_socket}', $fpm_socket, $content);
			$content = str_replace('{fpm_user}', 'rmnetdov', $content);
			$content = str_replace('{fpm_group}', 'rmnetdov', $content);
			wf($conf['nginx']['php_fpm_pool_dir'].'/rmnetdov.conf', $content);
		...
		if(is_file('/usr/local/bin/rmnetdov_update_from_dev.sh')) unlink('/usr/local/bin/rmnetdov_update_from_dev.sh');
...
//		chmod($install_dir.'/server/scripts/update_from_tgz.sh', 0700);
		chown($install_dir.'/server/scripts/rmnetdov_update.sh', 'root');
		chmod($install_dir.'/server/scripts/rmnetdov_update.sh', 0700);
		if(!is_link('/usr/local/bin/rmnetdov_update_from_dev.sh')) symlink($install_dir.'/server/scripts/rmnetdov_update.sh', '/usr/local/bin/rmnetdov_update_from_dev.sh');
		if(!is_link('/usr/local/bin/rmnetdov_update.sh')) symlink($install_dir.'/server/scripts/rmnetdov_update.sh', '/usr/local/bin/rmnetdov_update.sh');
		...
		//* Make the logs readable for the rmnetdov user
		...
		//* Create the rmnetdov log file and directory
		if(!is_file($conf['rmnetdov_log_dir'].'/rmnetdov.log')) {
			if(!is_dir($conf['rmnetdov_log_dir'])) mkdir($conf['rmnetdov_log_dir'], 0755);
			touch($conf['rmnetdov_log_dir'].'/rmnetdov.log');
		}
		chmod($conf['rmnetdov_log_dir'].'/rmnetdov.log', 0600);

		//* Create the rmnetdov auth log file and set uid/gid
		if(!is_file($conf['rmnetdov_log_dir'].'/auth.log')) {
			touch($conf['rmnetdov_log_dir'].'/auth.log');
		}
		exec('chown rmnetdov:rmnetdov '. $conf['rmnetdov_log_dir'].'/auth.log');
		exec('chmod 660 '. $conf['rmnetdov_log_dir'].'/auth.log');
		...
		if (is_dir('/etc/logrotate.d')) {
			@unlink('/etc/logrotate.d/logrmnetdov3'); // ignore, if the file is not there
			/* We rotate these logs in cron_daily.php
			$fh = fopen('/etc/logrotate.d/logrmnetdov3', 'w');
			fwrite($fh,
					"$conf['rmnetdov_log_dir']/rmnetdov.log { \n" .
					"	weekly \n" .
					"	missingok \n" .
					"	rotate 4 \n" .
					"	compress \n" .
					"	delaycompress \n" .
					"} \n" .
					"$conf['rmnetdov_log_dir']/cron.log { \n" .
					"	weekly \n" .
					"	missingok \n" .
					"	rotate 4 \n" .
					"	compress \n" .
		...
		if(@is_dir('/usr/local/rmnetdov/interface/web/domain')) exec('rm -rf /usr/local/rmnetdov/interface/web/domain');

		//* Disable rkhunter run and update in debian cronjob as rmnetdov is running and updating rkhunter
		...
		if(!is_link('/usr/local/bin/rmnetdov_patch')) exec('ln -s /usr/local/rmnetdov/server/scripts/rmnetdov_patch /usr/local/bin/rmnetdov_patch');
	...
	public function configure_dbserver() {
		...
		$content = rfsel($conf['rmnetdov_install_dir'].'/server/conf-custom/install/mysql_clientdb.conf.master', 'tpl/mysql_clientdb.conf.master');
	...
	public function install_crontab() {
		...
		// remove existing rmnetdov cronjobs, in case the syntax has changed
		foreach($existing_root_cron_jobs as $key => $val) {
			if(stristr($val, $install_dir)) unset($existing_root_cron_jobs[$key]);
		}

		$root_cron_jobs = array(
			"* * * * * ".$install_dir."/server/server.sh 2>&1 | while read line; do echo `/bin/date` \"\$line\" >> ".$conf['rmnetdov_log_dir']."/cron.log; done",
			"* * * * * ".$install_dir."/server/cron.sh 2>&1 | while read line; do echo `/bin/date` \"\$line\" >> ".$conf['rmnetdov_log_dir']."/cron.log; done"
		);
		...
		touch($conf['rmnetdov_log_dir'].'/cron.log');
		chmod($conf['rmnetdov_log_dir'].'/cron.log', 0660);

	}

	public function create_mount_script(){
		global $app, $conf;
		$mount_script = '/usr/local/rmnetdov/server/scripts/backup_dir_mount.sh';
		$mount_command = '';
	...
	// This function is called at the end of the update process and contains code to clean up parts of old RM-Net - DOV CP releases
	public function cleanup_rmnetdov() {
		global $app,$conf;

		// Remove directories recursively
		if(is_dir('/usr/local/rmnetdov/interface/web/designer')) exec('rm -rf /usr/local/rmnetdov/interface/web/designer');
		if(is_dir('/usr/local/rmnetdov/interface/web/themes/default-304')) exec('rm -rf /usr/local/rmnetdov/interface/web/themes/default-304');

		// Remove files
		if(is_file('/usr/local/rmnetdov/interface/lib/classes/db_firebird.inc.php')) unlink('/usr/local/rmnetdov/interface/lib/classes/db_firebird.inc.php');
		if(is_file('/usr/local/rmnetdov/interface/lib/classes/form.inc.php')) unlink('/usr/local/rmnetdov/interface/lib/classes/form.inc.php');
	 ...
	 * by inserting the rmnetdov database credentials using the {} placeholders.
	 *
	...
	protected function get_template_file($tLocal, $tRf=false, $tDBCred=false) {
		...
		$final_path = '';
		$dist_template = $conf['rmnetdov_install_dir'] . '/server/conf-custom/install/' . $tLocal . '.master';
	 ...
	 * file by inserting the common rmnetdov database
	 * credentials.
	 *
	 * @param $tContents
	 * @return string
	...
	protected function insert_db_credentials($tContents) {
		...
		$tContents = str_replace('{mysql_server_rmnetdov_user}', $conf["mysql"]["rmnetdov_user"], $tContents);
		$tContents = str_replace('{mysql_server_rmnetdov_password}', $conf["mysql"]["rmnetdov_password"], $tContents);
