...
function ilog($msg){
	exec("echo `date` \"- [RM-Net - DOV CP] - \"".$msg.' >> '.RMNETDOV_LOG_FILE);
}
...
function comment_out($file, $string){
	...
	foreach($modules as $val){
		...
		if(strstr($inhalt, $val)){
			$gesamt_inhalt = str_replace($val, '##RM-Net - DOV CP INSTALL## '.$val, $gesamt_inhalt);
		}
	}
...
/*
* Get the port number of the RM-Net - DOV CP controlpanel vhost
*/

function get_rmnetdov_port_number() {
	global $conf;
	if(is_file($conf['nginx']['vhost_conf_dir'].'/rmnetdov.vhost')) {
		$rmnetdov_vhost_file = $conf['nginx']['vhost_conf_dir'].'/rmnetdov.vhost';
		$regex = '/listen (\d+)/';
	} else {
		$rmnetdov_vhost_file = $conf['apache']['vhost_conf_dir'].'/rmnetdov.vhost';
		$regex = '/\<VirtualHost.*\:(\d{1,})\>/';
	}

	if(is_file($rmnetdov_vhost_file)) {
		$tmp = file_get_contents($rmnetdov_vhost_file);
...
/*
* Get the port number of the RM-Net - DOV CP apps vhost
*/

function get_apps_vhost_port_number() {
	global $conf;
	if(is_file($conf['nginx']['vhost_conf_dir'].'/apps.vhost')) {
		$rmnetdov_vhost_file = $conf['nginx']['vhost_conf_dir'].'/apps.vhost';
		$regex = '/listen (\d+)/';
	} else {
		$rmnetdov_vhost_file = $conf['apache']['vhost_conf_dir'].'/apps.vhost';
		$regex = '/\<VirtualHost.*\:(\d{1,})\>/';
	}
...
/*
 * Check if SSL is anabled in the RM-Net - DOV CP controlpanel vhost.
 */
function is_rmnetdov_ssl_enabled() {
	global $conf;
	$rmnetdov_vhost_file = $conf['apache']['vhost_conf_dir'].'/rmnetdov.vhost';
...
/*
* Is anonymization enabled in rmnetdov.conf file
*/

function get_logging_state() {
	global $conf;
	$rmnetdov_conf_file = $conf['apache']['vhost_conf_dir'].'/rmnetdov.conf';

	if(is_file($rmnetdov_conf_file)) {
		$tmp = file_get_contents($rmnetdov_conf_file);
		if(stristr($tmp, '/usr/local/rmnetdov/server/scripts/vlogger -p -s access.log')) {
			return 'anon';
		} elseif(stristr($tmp, '/usr/local/rmnetdov/server/scripts/vlogger -s access.log')) {
			return 'yes';
		} else {
			return 'no';
		}
