...
function prepareDBDump() {
	...
	if(is_file(RMNETDOV_INSTALL_ROOT."/install/sql/pre_update.sql")) {
		if($conf['mysql']['admin_password'] == '') {
			caselog("mysql --default-character-set=".escapeshellarg($conf['mysql']['charset'])." -h ".escapeshellarg($conf['mysql']['host'])." -u ".escapeshellarg($conf['mysql']['admin_user'])." ".escapeshellarg($conf['mysql']['database'])." < '".RMNETDOV_INSTALL_ROOT."/install/sql/pre_update.sql' &> /dev/null", __FILE__, __LINE__, 'read in pre_update.sql', 'could not read in pre_update.sql');
		} else {
			caselog("mysql --default-character-set=".escapeshellarg($conf['mysql']['charset'])." -h ".escapeshellarg($conf['mysql']['host'])." -u ".escapeshellarg($conf['mysql']['admin_user'])." -p".escapeshellarg($conf['mysql']['admin_password'])." ".escapeshellarg($conf['mysql']['database'])." < '".RMNETDOV_INSTALL_ROOT."/install/sql/pre_update.sql' &> /dev/null", __FILE__, __LINE__, 'read in pre_update.sql', 'could not read in pre_update.sql');
		}
	}
	...
	// create a backup copy of the rmnetdov database in the root folder
	if(isset($conf['backup_path'])) {
		$backup_db_name = $conf['backup_path'].'/rmnetdov_db_backup.sql';
	} else {
		$backup_db_name = '/root/rmnetdov_db_backup_'.@date('Y-m-d_H-i').'.sql';
	}
	...
	if ($conf['powerdns']['installed']) {
		...
		$backup_db_name = '/root/rmnetdov_powerdns_db_backup_'.@date('Y-m-d_h-i').'.sql';
...
function checkDbHealth() {
	...
	echo "Preverjanje baze podatkov RM-Net - DOV CP .. ";
...
function updateDbAndIni() {
	...
	//* Do incremental DB updates only on installed RM-Net - DOV CP versions >= 3.0.3
	if(version_compare('3.0.3', RMNETDOV_APP_VERSION, '<=')) {
		...
		while($found == true) {
			...
			if(is_file($sql_patch_filename)) {
				...
				if(in_array($next_db_version,explode(',',$silent_update_versions))) {
					...
					$cmd .= ' >> /var/log/rmnetdov_install.log 2>> /var/log/rmnetdov_install.log';
				}
				system($cmd);
		...
		//* If RM-Net - DOV CP Version < 3.0.3, we will do a full db update
	} else {
...
function check_service_config_state($servicename, $detected_value) {
	...
	if ($detected_value != $current_state) {
		...
		if ($answer == 'yes') return $detected_value;
			...
			if ($servicename == 'web_server') echo "\033[0;33mWARNING: If RM-Net - DOV CP-Interface (Webfrontend) is installed on this Server we will configure the Web Server anyways but will not enable it in RM-Net - DOV CP.\033[0m\n\n";
...
function checkAndRenameCustomTemplates($default_prompt='no') {
	...
	$template_directories = array(
		'/usr/local/rmnetdov/server/conf-custom',
		'/usr/local/rmnetdov/server/conf-custom/install',
	);
