...
When you add or modify a database field or table in the RM-Net - DOV CP database,
then follow these steps:

1) Add the field or table in the rmnetdov3.sql file. This file contains the
   complete database dump which is used when RM-Net - DOV CP gets installed.
...
2) Edit the file "incremental/upd_dev_collection.sql" which contains the SQL
   ...
   are required to modify the current rmnetdov database during update.

   The upd_dev_collection.sql file contains all db schema modifications
   for changes made since the last RM-Net - DOV CP release.  If SQL statements
   ...
   When a new RM-Net - DOV CP update is released, the contents of
...
Note: Incremental patches are supported for installed RM-Net - DOV CP versions > 3.0.3.
      If the installed version is < 3.0.3, then the full update method is used.
	  In other words, RM-Net - DOV CP 3.0.3 is the patch release (dbversion) 0 as the
