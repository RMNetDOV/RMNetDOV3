...
class installer_dist extends installer_base {
	...
	public function configure_saslauthd() {
		...
		$content = rfsel($conf['rmnetdov_install_dir'].'/server/conf-custom/install/fedora_saslauthd_smtpd_conf.master', $configfile);
...
//		if(is_dir('/etc/dovecot')) $config_dir = '/etc/dovecot';
		...
		if(version_compare($dovecot_version,2) >= 0) {
			if(is_file($conf['rmnetdov_install_dir'].'/server/conf-custom/install/fedora_dovecot2.conf.master')) {
				copy($conf['rmnetdov_install_dir'].'/server/conf-custom/install/fedora_dovecot2.conf.master', $config_dir.'/'.$configfile);
			} else {
			...
			if(is_file($conf['rmnetdov_install_dir'].'/server/conf-custom/install/fedora_dovecot.conf.master')) {
				copy($conf['rmnetdov_install_dir'].'/server/conf-custom/install/fedora_dovecot.conf.master', $config_dir.'/'.$configfile);
			} else {
		...
		$content = rfsel($conf['rmnetdov_install_dir'].'/server/conf-custom/install/fedora_dovecot-sql.conf.master', "tpl/fedora_dovecot-sql.conf.master");
		$content = str_replace('{mysql_server_rmnetdov_user}', $conf['mysql']['rmnetdov_user'], $content);
		$content = str_replace('{mysql_server_rmnetdov_password}', $conf['mysql']['rmnetdov_password'], $content);
		$content = str_replace('{mysql_server_database}', $conf['mysql']['database'], $content);
		$content = str_replace('{mysql_server_host}', $conf['mysql']['host'], $content);
	...
	public function configure_pureftpd()
	{
		...
		if(is_file($conf['rmnetdov_install_dir'].'/server/conf-custom/install/fedora_pureftpd_conf.master')) {
			exec("cp " . $conf['rmnetdov_install_dir'].'/server/conf-custom/install/fedora_pureftpd_conf.master ' . "$config_dir/pure-ftpd.conf");
		}else {
	...
	public function configure_nginx(){
		...
		wf('/etc/nginx/conf.d/rmnetdov_vhosts.conf', "include /etc/nginx/sites-enabled/*.vhost;");
	...
	public function install_rmnetdov()
	{
		...
		if($conf['apache']['installed'] == true && $this->install_rmnetdov_interface == true){
			...
			if(!is_dir('/var/www/php-fcgi-scripts/rmnetdov')) exec('mkdir -p /var/www/php-fcgi-scripts/rmnetdov');
			...
			if(!is_link('/var/www/rmnetdov')) exec('ln -s /usr/local/rmnetdov/interface/web /var/www/rmnetdov');
