...
class installer_dist extends installer_base {
	...
	public function __construct() {
		...
		if(in_array('authz_compat', $mods, true)) {
			...
			swriteln($inst->lng('    If it uses the old syntax (deny from all) RM-Net - DOV CP would fail to work.'));
		}
	}
	...
	function configure_postfix($options = '')
	{
		...
		$postconf_tpl = rfsel($conf['rmnetdov_install_dir'].'/server/conf-custom/install/opensuse_postfix.conf.master', 'tpl/opensuse_postfix.conf.master');
	...
	public function configure_dovecot()
	{
		...
		if($dovecot_version == 2) {
			if(is_file($conf['rmnetdov_install_dir'].'/server/conf-custom/install/opensuse_dovecot2.conf.master')) {
				copy($conf['rmnetdov_install_dir'].'/server/conf-custom/install/opensuse_dovecot2.conf.master', $config_dir.'/'.$configfile);
			} else {
			...
			if(is_file($conf['rmnetdov_install_dir'].'/server/conf-custom/install/opensuse_dovecot.conf.master')) {
				copy($conf['rmnetdov_install_dir'].'/server/conf-custom/install/opensuse_dovecot.conf.master', $config_dir.'/'.$configfile);
			} else {
		...
		$content = rfsel($conf['rmnetdov_install_dir'].'/server/conf-custom/install/opensuse_dovecot-sql.conf.master', "tpl/opensuse_dovecot-sql.conf.master");
		$content = str_replace('{mysql_server_rmnetdov_user}', $conf['mysql']['rmnetdov_user'], $content);
		$content = str_replace('{mysql_server_rmnetdov_password}', $conf['mysql']['rmnetdov_password'], $content);
		$content = str_replace('{mysql_server_database}', $conf['mysql']['database'], $content);
		$content = str_replace('{mysql_server_host}', $conf['mysql']['host'], $content);
	...
	public function configure_pureftpd()
	{
		...
		if(is_file($conf['rmnetdov_install_dir'].'/server/conf-custom/install/opensuse_pureftpd_conf.master')) {
			exec("cp " . $conf['rmnetdov_install_dir']."/server/conf-custom/install/opensuse_pureftpd_conf.master $config_dir/pure-ftpd.conf");
		} else {
	...
	public function configure_apache()
	{
		...
		exec('mkdir -p /var/log/rmnetdov/httpd');
		...
		if(!@is_link($vhost_conf_enabled_dir."/000-rmnetdov.conf")) {
			exec("ln -s ".$vhost_conf_dir."/rmnetdov.conf ".$vhost_conf_enabled_dir."/000-rmnetdov.conf");
		}
	...
	public function install_rmnetdov()
	{
		...
		if(is_file('/usr/local/rmnetdov/security/security_settings.ini~')) {
			$security_settings_old = ini_to_array(file_get_contents('/usr/local/rmnetdov/security/security_settings.ini~'));
			$security_settings_new = ini_to_array(file_get_contents('/usr/local/rmnetdov/security/security_settings.ini'));
			if(is_array($security_settings_new) && is_array($security_settings_old)) {
				...
				file_put_contents('/usr/local/rmnetdov/security/security_settings.ini',array_to_ini($security_settings_new));
			}
		}
		...
		if($conf['apache']['installed'] == true){
			$command = 'usermod -a -G rmnetdov '.$conf['apache']['user'];
			caselog($command.' &> /dev/null', __FILE__, __LINE__, "EXECUTED: $command", "Failed to execute the command $command");
			if(is_group('ispapps')){
		...
		if($conf['nginx']['installed'] == true){
			$command = 'usermod -a -G rmnetdov '.$conf['nginx']['user'];
			caselog($command.' &> /dev/null', __FILE__, __LINE__, "EXECUTED: $command", "Failed to execute the command $command");
			if(is_group('ispapps')){
		...
		if($conf['apache']['installed'] == true && $this->install_rmnetdov_interface == true){
			...
			//if(!is_file('/srv/www/php-fcgi-scripts/rmnetdov/.php-fcgi-starter')) {
			...
			exec('mkdir -p /srv/www/php-fcgi-scripts/rmnetdov');
			$this->set_immutable('/srv/www/php-fcgi-scripts/rmnetdov/.php-fcgi-starter', false);
			wf('/srv/www/php-fcgi-scripts/rmnetdov/.php-fcgi-starter', $content);
			exec('chmod +x /srv/www/php-fcgi-scripts/rmnetdov/.php-fcgi-starter');
			exec('ln -s /usr/local/rmnetdov/interface/web /srv/www/rmnetdov');
			exec('chown -R rmnetdov:rmnetdov /srv/www/php-fcgi-scripts/rmnetdov');
			$this->set_immutable('/srv/www/php-fcgi-scripts/rmnetdov/.php-fcgi-starter', true);
			...
			if($this->is_update == false) {
				...
				if(!@is_link("$vhost_conf_enabled_dir/000-rmnetdov.vhost")) {
					exec("ln -s $vhost_conf_dir/rmnetdov.vhost $vhost_conf_enabled_dir/000-rmnetdov.vhost");
				}

			}

			// Fix a setting in vhost master file for suse
			replaceLine('/usr/local/rmnetdov/server/conf/vhost.conf.master', "suPHP_UserGroup", "        suPHP_UserGroup <tmpl_var name='system_user'> <tmpl_var name='system_group'>", 0);
		}
		...
		// Make the Clamav log files readable by RM-Net - DOV CP
		...
		exec('chown root /usr/local/rmnetdov/server/scripts/update_from_dev.sh');
		exec('chmod 700 /usr/local/rmnetdov/server/scripts/update_from_dev.sh');
		exec('chown root /usr/local/rmnetdov/server/scripts/update_stable.sh');
		exec('chmod 700 /usr/local/rmnetdov/server/scripts/update_stable.sh');
		exec('chown root /usr/local/rmnetdov/server/scripts/rmnetdov_update.sh');
		exec('chmod 700 /usr/local/rmnetdov/server/scripts/rmnetdov_update.sh');
		if(!is_link('/usr/local/bin/rmnetdov_update_from_dev.sh')) exec('ln -s /usr/local/rmnetdov/server/scripts/rmnetdov_update.sh /usr/local/bin/rmnetdov_update_from_dev.sh');
		if(!is_link('/usr/local/bin/rmnetdov_update.sh')) exec('ln -s /usr/local/rmnetdov/server/scripts/rmnetdov_update.sh /usr/local/bin/rmnetdov_update.sh');
		...
		exec('chmod a+rx /usr/local/rmnetdov');
		exec('chmod -R 751 /usr/local/rmnetdov/interface');
		exec('chmod a+rx /usr/local/rmnetdov/interface/web');

		//* Create the rmnetdov log directory
		if(!is_dir($conf['rmnetdov_log_dir'])) mkdir($conf['rmnetdov_log_dir']);
		if(!is_file($conf['rmnetdov_log_dir'].'/rmnetdov.log')) exec('touch '.$conf['rmnetdov_log_dir'].'/rmnetdov.log');
		chmod($conf['rmnetdov_log_dir'].'/rmnetdov.log', 0600);

		if(is_user('getmail')) {
			exec('mv /usr/local/rmnetdov/server/scripts/run-getmail.sh /usr/local/bin/run-getmail.sh');
