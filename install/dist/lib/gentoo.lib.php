...
class installer extends installer_base
{
	...
	public function configure_postfix($options = '')
	{
		...
		$postconf_tpl = rfsel($conf['rmnetdov_install_dir'].'/server/conf-custom/install/gentoo_postfix.conf.master', 'tpl/gentoo_postfix.conf.master');
		...
		$content = rfsel($conf['rmnetdov_install_dir'].'/server/conf-custom/install/mailfilter.master', 'tpl/mailfilter.master');
	...
	public function configure_amavis()
	{
		...
		$conf_file = 'amavisd-rmnetdov.conf';
	...
	public function configure_apache()
	{
		...
		if (!is_dir($conf['rmnetdov_log_dir'].'/httpd')) {
			mkdir($conf['rmnetdov_log_dir'].'/httpd', 0755, true);
		}
		...
		//* Enable RM-Net - DOV CP default vhost settings
		...
		wf($conf['apache']['vhost_conf_dir'].'/000-rmnetdov.conf', $tpl->grab());
		unset($tpl);
	...
	public function install_rmnetdov()
	{
		...
		if($conf['apache']['installed'] == true && $this->install_rmnetdov_interface == true){
			...
			$content = $this->get_template_file("apache_rmnetdov.vhost", true);
			...
			$vhost_path = $conf['apache']['vhost_conf_dir'].'/rmnetdov.vhost';
			...
			if(!is_file('/var/www/php-fcgi-scripts/rmnetdov/.php-fcgi-starter')) {
				...
				chmod('/var/www/php-fcgi-scripts/rmnetdov/.php-fcgi-starter', 0755);
		...
		if (is_file('/usr/local/bin/rmnetdov_update_from_dev.sh')) {
			unlink('/usr/local/bin/rmnetdov_update_from_dev.sh');
		}
		...
		if (!is_link('/usr/local/bin/rmnetdov_update_from_dev.sh')) {
			symlink($install_dir.'/server/scripts/rmnetdov_update.sh', '/usr/local/bin/rmnetdov_update_from_dev.sh');
		}

		if (!is_link('/usr/local/bin/rmnetdov_update.sh')) {
			symlink($install_dir.'/server/scripts/rmnetdov_update.sh', '/usr/local/bin/rmnetdov_update.sh');
		}
