...
class webserver_plugin {
	...
	public function check_phpini_changes() {
		...
		if($web_config['php_ini_check_minutes'] == 0 || @date('i') % $web_config['php_ini_check_minutes'] != 0) {
			$app->log('Informacije: preverjanje sprememb php.ini trenutno ni omogočeno ali ne: ' . $web_config['php_ini_check_minutes'], LOGLEVEL_DEBUG);
	...
	function server_php_update($event_name, $data) {
		...
		if(isset($data['new']['php_fastcgi_ini_dir'])) {
			...
			if(file_exists($php_ini)) {
				...
				$app->log("Ni mogoče preveriti php.ini, datoteke ni mogoče najti: $php_ini", LOGLEVEL_WARN);
			}
		}
	...
	function server_update($event_name, $data) {
		...
		{
			$app->log('Konfiguracija Jailkita je bila spremenjena, načrtovanje posodobitev prizadetih zapornikov chroot.', LOGLEVEL_DEBUG);
