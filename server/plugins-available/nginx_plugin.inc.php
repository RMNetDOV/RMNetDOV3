...
class nginx_plugin {
	...
	function onInstall() {
		...
		if($conf['services']['web'] == true && !@is_link('/usr/local/rmnetdov/server/plugins-enabled/apache2_plugin.inc.php')) {
			return true;
		} else {
			return false;
		}
	...
	*/
	...
	function update($event_name, $data) {
		...
		if(@is_link('/usr/local/rmnetdov/server/plugins-enabled/apache2_plugin.inc.php')) {
		...
		if($web_config['connect_userid_to_webid'] == 'y') {
			...
			//* Check if a rmnetdovend user and group exists and create them
			if(!$app->system->is_group('rmnetdovend')) {
				$app->system->exec_safe('groupadd --gid ? rmnetdovend', $connect_userid_to_webid_start + 10000);
			}
			if(!$app->system->is_user('rmnetdovend')) {
				$app->system->exec_safe('useradd -g rmnetdovend -d /usr/local/rmnetdov --uid ? rmnetdovend', $connect_userid_to_webid_start + 10000);
			}
		} else {
			$fixed_uid_param = '';
			$fixed_gid_param = '';
		...
		if($this->action == 'update' && $data['new']['document_root'] != $data['old']['document_root']) {
			...
			$fstab_line_old = '/var/log/rmnetdov/httpd/'.$data['old']['domain'].' '.$data['old']['document_root'].'/'.$old_log_folder.'    none    bind';

			if($web_config['network_filesystem'] == 'y') {
				$fstab_line = '/var/log/rmnetdov/httpd/'.$data['new']['domain'].' '.$data['new']['document_root'].'/'.$log_folder.'    none    bind,nofail,_netdev    0 0';
				$app->system->replaceLine('/etc/fstab', $fstab_line_old, $fstab_line, 0, 1);
			} else {
				$fstab_line = '/var/log/rmnetdov/httpd/'.$data['new']['domain'].' '.$data['new']['document_root'].'/'.$log_folder.'    none    bind,nofail    0 0';
				$app->system->replaceLine('/etc/fstab', $fstab_line_old, $fstab_line, 0, 1);
			}

			$app->system->exec_safe('mount --bind ? ?', '/var/log/rmnetdov/httpd/'.$data['new']['domain'], $data['new']['document_root'].'/'.$log_folder);
		...
		if($this->action == 'update' && $data['old']['domain'] != '' && $data['new']['domain'] != $data['old']['domain']) {
			if(is_dir('/var/log/rmnetdov/httpd/'.$data['old']['domain'])) $app->system->exec_safe('rm -rf ?', '/var/log/rmnetdov/httpd/'.$data['old']['domain']);
			...
			$fstab_line = '/var/log/rmnetdov/httpd/'.$data['old']['domain'].' '.$data['old']['document_root'].'/'.$old_log_folder.'    none    bind';
		...
		if(!is_dir($data['new']['document_root'].'/'.$log_folder) || !is_dir('/var/log/rmnetdov/httpd/'.$data['new']['domain']) || is_link($data['new']['document_root'].'/'.$log_folder)) {
			if(is_link($data['new']['document_root'].'/'.$log_folder)) unlink($data['new']['document_root'].'/'.$log_folder);
			if(!is_dir('/var/log/rmnetdov/httpd/'.$data['new']['domain'])) $app->system->exec_safe('mkdir -p ?', '/var/log/rmnetdov/httpd/'.$data['new']['domain']);
			...
			$fstab_line = '/var/log/rmnetdov/httpd/'.$data['new']['domain'].' '.$data['new']['document_root'].'/'.$log_folder.'    none    bind,nofail';
		...
		// /usr/local/rmnetdov/server/conf is for the standard files
		// /usr/local/rmnetdov/server/conf-custom is for the custom files
		// setting a local var here

		// normally $conf['templates'] = "/usr/local/rmnetdov/server/conf";
		...
		if(($this->action == 'insert' && $data['new']['type'] == 'vhost') or ($web_config['set_folder_permissions_on_update'] == 'y' && $data['new']['type'] == 'vhost') or ($web_folder != $old_web_folder && $data['new']['type'] == 'vhost')) {
			...
			if($web_config['security_level'] == 20) {
				...
				if(realpath($data['new']['document_root'].'/'.$log_folder . '/error.log') == '/var/log/rmnetdov/httpd/'.$data['new']['domain'].'/error.log') {
					$app->system->chmod($data['new']['document_root'].'/'.$log_folder, 0755);
				}
		...
		if($data['new']['type'] == 'vhost') {
			...
			if(!@is_file('/var/log/rmnetdov/httpd/'.$data['new']['domain'].'/error.log')) $app->system->exec_safe('touch ?', '/var/log/rmnetdov/httpd/'.$data['new']['domain'].'/error.log');
			$app->system->chown('/var/log/rmnetdov/httpd/'.$data['new']['domain'].'/error.log', 'root');
			$app->system->chgrp('/var/log/rmnetdov/httpd/'.$data['new']['domain'].'/error.log', 'root');
		}
		...
		// backwards compatibility; since RM-Net - DOV 3.0.5, the PHP mode for nginx is called 'php-fpm' instead of 'fast-cgi'. The following line makes sure that old web sites that have 'fast-cgi' in the database still get PHP-FPM support.
...
//			$vhost_data['enable_pagespeed'] = false;
	...
	function delete($event_name, $data) {
				...
				if(is_array($bind_mount_lines) && !empty($bind_mount_lines)){
					foreach($bind_mount_lines as $bind_mount_line){
						...
						if(is_array($bind_mount_parts) && !empty($bind_mount_parts)){
							if($bind_mount_parts[0] == '/var/log/rmnetdov/httpd/'.$data['old']['domain'] && $bind_mount_parts[2] == 'none' && strpos($bind_mount_parts[3], 'bind') !== false){
		...
		if(is_array($log_folders) && !empty($log_folders)){
			foreach($log_folders as $log_folder){
				$fstab_line = '/var/log/rmnetdov/httpd/'.$data['old']['domain'].' '.$data['old']['document_root'].'/'.$log_folder.'    none    bind';
				$app->system->removeLine('/etc/fstab', $fstab_line);
			}
		} else {
			$fstab_line = '/var/log/rmnetdov/httpd/'.$data['old']['domain'].' '.$data['old']['document_root'].'/'.$log_folder.'    none    bind';
		...
		if($data['old']['type'] != 'vhost' && $data['old']['type'] != 'vhostsubdomain' && $data['old']['type'] != 'vhostalias' && $data['old']['parent_domain_id'] > 0) {
			...
			$vhost_logfile_dir = '/var/log/rmnetdov/httpd/'.$data['old']['domain'];
	...
	private function goaccess_update ($data, $web_config) {
		...
		$goaccess_conf = $data['new']['document_root'].'/log/goaccess.conf';
                ...
                /*
                In case that you use a different log format, you should use a custom goaccess.conf which you'll have to put into /usr/local/rmnetdov/server/conf-custom/.
                ...
                */
                ...
                if(file_exists("/usr/local/rmnetdov/server/conf-custom/goaccess.conf.master")) {
                        $app->system->copy("/usr/local/rmnetdov/server/conf-custom/goaccess_index.php.master", $goaccess_conf);
                        ...
                        /*
                         ...
                         Also there was no /usr/local/rmnetdov/server/conf-custom/goaccess.conf.master, so we gonna use /etc/goaccess.conf as the base conf.
                         */
                ...
                if(file_exists("/usr/local/rmnetdov/server/conf-custom/goaccess_index.php.master")) {
                        $app->system->copy("/usr/local/rmnetdov/server/conf-custom/goaccess_index.php.master", $data['new']['document_root']."/" . $web_folder . "/stats/index.php");
                } else {
                        $app->system->copy("/usr/local/rmnetdov/server/conf/goaccess_index.php.master", $data['new']['document_root']."/" . $web_folder . "/stats/index.php");
		}
	}
	...
	private function awstats_update ($data, $web_config) {
		...
		if(!@is_file($awstats_conf_dir.'/awstats.'.$data['new']['domain'].'.conf') || ($data['old']['domain'] != '' && $data['new']['domain'] != $data['old']['domain'])) {
			...
			$content .= "LogFile=\"/var/log/rmnetdov/httpd/".$data['new']['domain']."/access.log\"\n";
		...
		if(file_exists("/usr/local/rmnetdov/server/conf-custom/awstats_index.php.master")) {
			$app->system->copy("/usr/local/rmnetdov/server/conf-custom/awstats_index.php.master", $data['new']['document_root']."/" . $web_folder . "/stats/index.php");
		} else {
			$app->system->copy("/usr/local/rmnetdov/server/conf/awstats_index.php.master", $data['new']['document_root']."/" . $web_folder . "/stats/index.php");
		}
	}
