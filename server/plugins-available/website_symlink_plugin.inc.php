...
class website_symlink_plugin {
	...
	private function createSymlink($target, $link) {
		...
		if (file_exists($link)) {
			$app->log(sprintf('Ne ustvarjam simbolne povezave, ker "%s" že obstaja', $link), LOGLEVEL_DEBUG);

			return;
		}

		if ($app->system->create_relative_link($target, $link)) {
			$app->log(sprintf('Ustvarjena simbolna povezava od "%s" do "%s"', $link, $target), LOGLEVEL_DEBUG);
		} else {
			$app->log(sprintf('Ni bilo mogoče ustvariti simbolne povezave od "%s" do "%s"', $link, $target), LOGLEVEL_WARN);
		}
	}
}