	...
	*/
	...
	function insert($event_name, $data) {
		...
		if($security_config['allow_shell_user'] != 'yes') {
			$app->log('Uporabniški vtičnik Shell onemogočen zaradi varnostnih nastavitev.',LOGLEVEL_WARN);
			return false;
		}
		...
		if(!$app->system->is_allowed_user($data['new']['username'], false, false)
			...
			$app->log('Uporabnik lupine ne sme biti root ali v root skupine.',LOGLEVEL_WARN);
			return false;
		}

		if(is_file($data['new']['dir']) || is_link($data['new']['dir'])) {
			$app->log('Ime uporabnika lupine ne sme biti obstoječa datoteka ali simbolna povezava.', LOGLEVEL_WARN);
			return false;
		} elseif(!$app->system->is_allowed_path($data['new']['dir'])) {
			$app->log('Ime uporabnika lupine ni dovoljena pot: ' . $data['new']['dir'], LOGLEVEL_WARN);
			return false;
		}
		...
		if($app->system->is_user($data['new']['puser'])) {
			...
			$app->log("Preskok vstavljanja uporabnika:".$data['new']['username'].", parent user ".$data['new']['puser']." does not exist.", LOGLEVEL_WARN);
		}
	...
	function delete($event_name, $data) {
		...
		if(is_file($data['old']['dir']) || is_link($data['old']['dir'])) {
			...
			$app->log('Ime uporabnika lupine ni dovoljena pot: ' . $data['old']['dir'], LOGLEVEL_WARN);
			return false;
		}
	...
	function _setup_jailkit_chroot()
	{
		...
		if (!is_dir($this->data['new']['dir'].'/etc/jailkit'))
		{
			...
			$app->log("Dodan skript bashrc: ".$bashrc, LOGLEVEL_DEBUG);
	...
	function _add_jailkit_user()
	{
		...
		$app->log("Dodan domači starševski uporabnik jailkit : ".$this->data['new']['dir'].$jailkit_chroot_puserhome, LOGLEVEL_DEBUG);
	...
	private function _setup_ssh_rsa() {
		...
		if ($userkey == '' && $id > 0){
			...
			$app->log("par ključev ssh-rsa, ustvarjen za ".$username, LOGLEVEL_DEBUG);
		};
		...
		//* merge the remaining keys and the ones fom the rmnetdov database.
