    ...
    */
    ...
    function ssl($event_name, $data) {
        ...
        if($data['new']['ssl_action'] == 'create' && $conf['mirror_server_id'] == 0) {
            ...
            $app->log('Ustvarjanje potrdila XMPP SSL za: '.$domain, LOGLEVEL_DEBUG);
        }
        ...
        if($data["new"]["ssl_action"] == 'save') {
            ...
            /* Update also the master-DB of the Server-Farm */
            $app->dbmaster->query("UPDATE xmpp_domain SET ssl_action = '' WHERE domain = ?", $data['new']['domain']);
            $app->log('Shranjevanje potrdila XMPP SSL za: '.$domain, LOGLEVEL_DEBUG);
        }
        ...
        if($data['new']['ssl_action'] == 'del') {
            ...
            $app->dbmaster->query("UPDATE xmpp_domain SET ssl_action = '' WHERE domain = ?", $data['new']['domain']);
            $app->log('Brisanje potrdila SSL za: '.$domain, LOGLEVEL_DEBUG);
        }
