...
class powerdns_plugin {
	...
	*/
	...
	function soa_insert($event_name, $data) {
		...
		$app->db->query("INSERT INTO powerdns.domains (name, type, notified_serial, rmnetdov_id) VALUES (?, ?, ?, ?)", $origin, 'MASTER', $serial_id, $rmnetdov_id);
		...
		$app->db->query("INSERT INTO powerdns.records (domain_id, name, type, content, ttl, prio, change_date, rmnetdov_id) VALUES (?, ?, 'SOA', ?, ?, 0, UNIX_TIMESTAMP(), ?)", $zone_id, $origin, $content, $ttl, $rmnetdov_id);
	...
	function soa_update($event_name, $data) {
		...
		if($data["new"]["active"] != 'Y'){
			...
			$exists = $app->db->queryOneRecord("SELECT * FROM powerdns.domains WHERE rmnetdov_id = ?", $data["new"]["id"]);
			if($data["old"]["active"] == 'Y' && is_array($exists)){
				...
				$content = $ns.' '.$hostmaster.' '.$data["new"]["serial"].' '.$data["new"]["refresh"].' '.$data["new"]["retry"].' '.$data["new"]["expire"].' '.$data["new"]["minimum"];
				$ttl = $data["new"]["ttl"];
				$app->db->query("UPDATE powerdns.records SET name = ?, content = ?, ttl = ?, change_date = UNIX_TIMESTAMP() WHERE rmnetdov_id = ? AND type = 'SOA'", $origin, $content, $ttl, $data["new"]["id"]);
	...
	function soa_delete($event_name, $data) {
		...
		$zone = $app->db->queryOneRecord("SELECT * FROM powerdns.domains WHERE rmnetdov_id = ? AND type = 'MASTER'", $data["old"]["id"]);
		$zone_id = $zone["id"];
	...
	function slave_insert($event_name, $data) {
		...
		$app->db->query("INSERT INTO powerdns.domains (name, type, master, rmnetdov_id) VALUES (?, ?, ?, ?)", $origin, 'SLAVE', $master_ns, $rmnetdov_id);
	...
	function slave_update($event_name, $data) {
		...
		if($data["new"]["active"] != 'Y'){
			...
			if($data["old"]["active"] == 'Y'){
				...
				$app->db->query("UPDATE powerdns.domains SET name = ?, type = 'SLAVE', master = ? WHERE rmnetdov_id=? AND type = 'SLAVE'", $origin, $master_ns, $rmnetdov_id);
				$zone_id = $app->db->insertID();

				$zone = $app->db->queryOneRecord("SELECT * FROM powerdns.domains WHERE rmnetdov_id = ? AND type = 'SLAVE'", $rmnetdov_id);
				$zone_id = $zone["id"];
				$app->db->query("DELETE FROM powerdns.records WHERE domain_id = ? AND rmnetdov_id = 0", $zone_id);
	...
	function slave_delete($event_name, $data) {
		...
		$zone = $app->db->queryOneRecord("SELECT * FROM powerdns.domains WHERE rmnetdov_id = ? AND type = 'SLAVE'", $data["old"]["id"]);
		$zone_id = $zone["id"];
		$app->db->query("DELETE FROM powerdns.records WHERE domain_id = ?", $zone_id);
		$app->db->query("DELETE FROM powerdns.domains WHERE id = ?", $zone_id);
	}
	...
	function rr_insert($event_name, $data) {
		global $app, $conf;
		if($data["new"]["active"] != 'Y') return;
		$exists = $app->db->queryOneRecord("SELECT * FROM powerdns.records WHERE rmnetdov_id = ?", $data["new"]["id"]);
		...
		$powerdns_zone = $app->db->queryOneRecord("SELECT * FROM powerdns.domains WHERE rmnetdov_id = ? AND type = 'MASTER'", $data["new"]["zone"]);
		...
		$app->db->query("INSERT INTO powerdns.records (domain_id, name, type, content, ttl, prio, change_date, rmnetdov_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?)", $zone_id, $name, $type, $content, $ttl, $prio, $change_date, $rmnetdov_id);
	...
	function rr_update($event_name, $data) {
		...
		if($data["new"]["active"] != 'Y'){
			...
			if($data["old"]["active"] == 'Y' && is_array($exists)){
				...
				$change_date = time();
				$rmnetdov_id = $data["new"]["id"];
				$app->db->query("UPDATE powerdns.records SET name = ?, type = ?, content = ?, ttl = ?, prio = ?, change_date = UNIX_TIMESTAMP() WHERE rmnetdov_id = ? AND type != 'SOA'", $name, $type, $content, $ttl, $prio, $rmnetdov_id);
	...
	function rr_delete($event_name, $data) {
		...
		$rmnetdov_id = $data["old"]["id"];
		$app->db->query("DELETE FROM powerdns.records WHERE rmnetdov_id = ? AND type != 'SOA'", $rmnetdov_id);
	}
	...
	function rectifyZone($data) {
		...
		if ( $pdns_pdnssec != false ) {
			if (isset($data["new"]["origin"])) {
				...
				$zn = $app->db->queryOneRecord("SELECT d.name AS name FROM powerdns.domains d, powerdns.records r WHERE r.rmnetdov_id=? AND r.domain_id = d.id", $data["new"]["id"]);
