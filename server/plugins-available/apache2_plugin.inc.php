	...
	*/
	...
	function update($event_name, $data) {
		...
		if($data['new']['type'] == 'vhost') {
			...
			if(!@is_file('/var/log/rmnetdov/httpd/'.$data['new']['domain'].'/error.log')) {
				$app->system->exec_safe('touch ?', '/var/log/rmnetdov/httpd/'.$data['new']['domain'].'/error.log');
			}
	...
	function server_ip($event_name, $data) {
		...
		$tpl = new tpl();
		$tpl->newTemplate('apache_rmnetdov.conf.master');
		$tpl->setVar('apache_version', $app->system->getapacheversion());
		...
		$vhost_file = $web_config['vhost_conf_dir'].'/rmnetdov.conf';
		$app->system->file_put_contents($vhost_file, $tpl->grab());
	...
	function web_folder_user($event_name, $data) {
		...
		$begin_marker = '### RM-Net - DOV CP folder protection begin ###';
		$end_marker = "### RM-Net - DOV CP folder protection end ###\n\n";
