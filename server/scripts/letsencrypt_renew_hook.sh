...
# Description:  Taken from LE4RMNETDOV code. To be used to update rmnetdovserver.pem automatically after RM-Net - DOV LE SSL certs are renewed and to reload / restart important RM-Net - DOV server services
### END INIT INFO

## If you need a custom hook file, create a file with the same name in
## /usr/local/rmnetdov/server/conf-custom/scripts/
##
## End the file with 'return 124' to signal that this script should not terminate.
if [ -e "/usr/local/rmnetdov/server/conf-custom/scripts/letsencrypt_renew_hook.sh" ] ; then
        . /usr/local/rmnetdov/server/conf-custom/scripts/letsencrypt_renew_hook.sh
        ret=$?
        if [ $ret != 124 ]; then exit $ret; fi
fi

hostname=$(hostname -f)
if [ -d "/usr/local/rmnetdov/server/scripts/${hostname}" ] ; then
	lelive="/usr/local/rmnetdov/server/scripts/${hostname}" ;
...
if [ -d "$lelive" ]; then
    cd /usr/local/rmnetdov/interface/ssl; ibak=rmnetdovserver.*.bak; ipem=rmnetdovserver.pem; icrt=rmnetdovserver.crt; ikey=rmnetdovserver.key
...
else echo `/bin/date` "Your Lets Encrypt SSL certs path for your RM-Net - DOV server FQDN is missing.$line" >> /var/log/rmnetdov/rmnetdov.log; fi
