...
class cronjob_mailbox_stats extends cronjob {
	...
	public function onRunJob() {
		...
		foreach($records as $rec) {
			if(@is_file($rec['maildir'].'/rmnetdov_mailsize')) {
				$parse_mail_log = false;

				// rename file
				rename($rec['maildir'].'/rmnetdov_mailsize', $rec['maildir'].'/rmnetdov_mailsize_save');

				// Read the file
				$lines = file($rec['maildir'].'/rmnetdov_mailsize_save');
				$mail_traffic = 0;
				...
				// Delete backup file
				if(@is_file($rec['maildir'].'/rmnetdov_mailsize_save')) unlink($rec['maildir'].'/rmnetdov_mailsize_save');
