...
class cronjob_logfiles extends cronjob {
	...
	public function onRunJob() {
		...
		if(is_dir('/var/log/rmnetdov/httpd')) exec('chmod +r /var/log/rmnetdov/httpd/*');
		...
		//* Delete old logfiles in /var/log/rmnetdov/httpd/ that were created by vlogger for the hostname of the server
		exec('hostname -f', $tmp_hostname);
		if($tmp_hostname[0] != '' && is_dir('/var/log/rmnetdov/httpd/'.$tmp_hostname[0])) {
			$app->system->exec_safe("cd ?; find . -mtime +$max_syslog -name '*.log' | xargs rm > /dev/null 2> /dev/null", '/var/log/rmnetdov/httpd/'.$tmp_hostname[0]);
		}
		unset($tmp_hostname);

		//######################################################################################################
		// Rotate the rmnetdov.log file
		//######################################################################################################


		$rmnetdov_logfiles = array('rmnetdov.log', 'cron.log', 'auth.log', 'acme.log');
		foreach($rmnetdov_logfiles as $rmnetdov_logfile) {
			$num = $max_syslog;
			$rmnetdov_logfile = $conf['rmnetdov_log_dir'].'/'.$rmnetdov_logfile;
			// rename older files (move up by one)
			while($num >= 1) {
				if(is_file($rmnetdov_logfile . '.' . $num . '.gz')) rename($rmnetdov_logfile . '.' . $num . '.gz', $rmnetdov_logfile . '.' . ($num + 1) . '.gz');
				$num--;
			}
			//* compress current logfile
			if(is_file($rmnetdov_logfile)) {
				$app->system->exec_safe("gzip -c ? > ?", $rmnetdov_logfile, $rmnetdov_logfile . '.1.gz');
				$app->system->exec_safe("cat /dev/null > ?", $rmnetdov_logfile);
			}
			// remove older logs
			$num = $max_syslog;
			while(is_file($rmnetdov_logfile . '.' . $num . '.gz')) {
				@unlink($rmnetdov_logfile . '.' . $num . '.gz');
				$num++;
			}
		}
