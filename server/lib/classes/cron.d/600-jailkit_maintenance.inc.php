...
class cronjob_jailkit_maintenance extends cronjob {
	...
	public function onRunJob() {
		...
		if (! is_file('/usr/local/rmnetdov/server/temp/jailkit_force_update.ts')) {
			if(!@is_dir('/usr/local/rmnetdov/server/temp')) {
				$app->system->mkdirpath('/usr/local/rmnetdov/server/temp');
			}
			$app->system->touch('/usr/local/rmnetdov/server/temp/jailkit_force_update.ts');
		} elseif ( time() - filemtime('/usr/local/rmnetdov/server/temp/jailkit_force_update.ts') > 60 * 60 * 24 * 14 ) {
