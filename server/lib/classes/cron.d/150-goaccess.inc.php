			...
			if($app->system->is_installed('goaccess')) {
				...
				foreach($records as $rec) {
					...
					*/
					...
					if(file_exists("/usr/local/rmnetdov/server/conf-custom/goaccess.conf.master") && (!file_exists($goaccess_conf))) {
						$app->system->copy("/usr/local/rmnetdov/server/conf-custom/goaccess.conf.master", $goaccess_conf);
						...
						/*
						 ...
						 Also there was no /usr/local/rmnetdov/server/conf-custom/goaccess.conf.master, so we gonna use /etc/goaccess.conf or /etc/goaccess/goaccess.conf as the base conf.
						*/
					...
					$output_html = $docroot.'/'.$web_folder.'/stats/goaindex.html';
		                        ...
		                        if(is_link('/var/log/rmnetdov/httpd/'.$domain.'/yesterday-access.log')) $app->system->unlink('/var/log/rmnetdov/httpd/'.$domain.'/yesterday-access.log');
					...
					if(!is_file($rec['document_root']."/".$web_folder."/stats/index.php")) {
						if(file_exists("/usr/local/rmnetdov/server/conf-custom/goaccess_index.php.master")) {
							$app->system->copy("/usr/local/rmnetdov/server/conf-custom/goaccess_index.php.master", $rec['document_root']."/".$web_folder."/stats/index.php");
						} else {
							$app->system->copy("/usr/local/rmnetdov/server/conf/goaccess_index.php.master", $rec['document_root']."/".$web_folder."/stats/index.php");
						}
					}
