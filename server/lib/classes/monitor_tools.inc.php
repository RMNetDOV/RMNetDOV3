...
//		}
	...
	public function _getLogData($log, $max_lines = 100) {
		...
		case 'log_rmnetdov_cron':
			if ($dist == 'debian') {
				$logfile = $conf['rmnetdov_log_dir'] . '/cron.log';
			} elseif ($dist == 'redhat') {
				$logfile = $conf['rmnetdov_log_dir'] . '/cron.log';
			} elseif ($dist == 'suse') {
				$logfile = $conf['rmnetdov_log_dir'] . '/cron.log';
		...
		case 'log_letsencrypt':
				$check_files = array();
				if(file_exists($conf['rmnetdov_log_dir'].'/acme.log')) {
					$check_files[] = $conf['rmnetdov_log_dir'].'/acme.log';
				}
				if(file_exists('/root/.acme.sh/acme.sh') && file_exists('/root/.acme.sh/acme.sh.log')) {
					$check_files[] = '/root/.acme.sh/acme.sh.log';
				}
				if(file_exists('/usr/local/rmnetdov/server/scripts/acme.sh') && file_exists('/usr/local/rmnetdov/server/scripts/acme.sh.log')) {
					$check_files[] = '/usr/local/rmnetdov/server/scripts/acme.sh.log';
				}
		...
		case 'log_rmnetdov':
			if ($dist == 'debian') {
				$logfile = $conf['rmnetdov_log_dir'] . '/rmnetdov.log';
			} elseif ($dist == 'redhat') {
				$logfile = $conf['rmnetdov_log_dir'] . '/rmnetdov.log';
			} elseif ($dist == 'suse') {
				$logfile = $conf['rmnetdov_log_dir'] . '/rmnetdov.log';
	...
	private function _checkTcp($host, $port) {
		...
		if ($fp) {
			...
			if ($port == 80) {
				...
				$out = "GET / HTTP/1.1\r\n";
				$out .= "Host: localhost\r\n";
				$out .= "User-Agent: Mozilla/5.0 (RM-Net - DOV monitor)\r\n";
