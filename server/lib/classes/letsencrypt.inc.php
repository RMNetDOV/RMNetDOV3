...
class letsencrypt {
	...
	public function get_acme_command($domains, $key_file, $bundle_file, $cert_file, $server_type = 'apache') {
		...
		$cmd = 'R=0 ; C=0 ; ' . $letsencrypt . ' --issue ' . $cmd . ' -w /usr/local/rmnetdov/interface/acme --always-force-new-domain-key --keylength 4096; R=$? ; if [[ $R -eq 0 || $R -eq 2 ]] ; then ' . $letsencrypt . ' --install-cert ' . $cmd . ' --key-file ' . escapeshellarg($key_file) . ' ' . $cert_arg . ' --reloadcmd ' . escapeshellarg($this->get_reload_command()) . ' --log ' . escapeshellarg($conf['rmnetdov_log_dir'].'/acme.log') . '; C=$? ; fi ; if [[ $C -eq 0 ]] ; then exit $R ; else exit $C  ; fi';
	...
	public function get_certbot_command($domains) {
		...
		if (version_compare($letsencrypt_version, '0.30', '>=')) {
			...
			for($i = 0; $i < count($domains); $i++) {
				$webroot_map[$domains[$i]] = '/usr/local/rmnetdov/interface/acme';
			}
			...
			$webroot_args = "$cmd --webroot-path /usr/local/rmnetdov/interface/acme";
	...
	public function request_certificates($data, $server_type = 'apache') {
		...
		if(!is_dir('/usr/local/rmnetdov/interface/acme/.well-known/acme-challenge/')) {
			$app->system->mkdir('/usr/local/rmnetdov/interface/acme/.well-known/acme-challenge/', false, 0755, true);
		}
		file_put_contents('/usr/local/rmnetdov/interface/acme/.well-known/acme-challenge/' . $le_rnd_file, $le_rnd_hash);
		...
		unset($le_domains);
		@unlink('/usr/local/rmnetdov/interface/acme/.well-known/acme-challenge/' . $le_rnd_file);
