...
class system{
	...
	var $FILE = '/root/rmnetdov/scripts/lib/classes/rmnetdov_system.lib.php';
	var $server_id;
	var $server_conf;
	var $data;
	var $min_uid = 500;
	...
	*/
	 ...
	 *
	 */
	function add_user_to_group($group, $user = 'admrmnetdov'){
		global $app;
		$group_file = $app->file->rf($this->server_conf['group_datei']);
		$group_file_lines = explode("\n", $group_file);
		foreach($group_file_lines as $group_file_line){
	...
	*/
	...
	function mount_backup_dir($backup_dir, $mount_cmd = '/usr/local/rmnetdov/server/scripts/backup_dir_mount.sh'){
		global $app, $conf;
	...
	function umount_backup_dir($backup_dir, $mount_cmd = '/usr/local/rmnetdov/server/scripts/backup_dir_umount.sh'){
		global $app, $conf;
	...
	public function is_allowed_user($username, $check_id = true, $restrict_names = false) {
		...
		$name_blacklist = array('root','rmnetdov','vmail','getmail');
...
$app->log("update_jailkit_chroot: removing deprecated directory which jk_update failed to remove:  ".$matches[1], LOGLEVEL_DEBUG);
					...
					$app->log("jk_update error for jail $home_dir, feel free to pass to rmnetdov developers:  ".print_r( $matches, true), LOGLEVEL_DEBUG);
				}
