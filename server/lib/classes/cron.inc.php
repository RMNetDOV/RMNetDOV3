...
class cron {
	...
	public function getNextRun($vDate) {
		...
		$iTimestamp = RMNetDOVDatetime::to_timestamp($vDate);
		if($iTimestamp === false) return $iTimestamp;

		if($this->_bParsed == false) $this->_calcValidValues();

		// get the field values for the given Date.
		list($iMinute, $iHour, $iDay, $iWDay, $iMonth, $iYear) = explode(':', RMNetDOVDateTime::to_string($vDate, 'custom:%M:%H:%d:%w:%m:%Y'));
		...
		while($bValid == false) {
			...
			if($iNextMinute > $iMinute && $iHour == $iCurHour && $iDay == $iCurDay && $iWDay == $iCurWDay && $iMonth == $iCurMonth) {
				...
				$iHour = $iNextHour;
			} elseif($iNextDay > $iDay && RMNetDOVDateTime::last_day($iMonth) >= $iNextDay && $iMonth == $iCurMonth) {
				$iMinute = reset($this->_aValidValues['minute']);
			...
			if(RMNetDOVDateTime::last_day($iMonth, $iYear) >= $iDay && in_array($app->functions->intval(strftime('%w', $ts)), $this->_aValidValues['weekday'], true) === true) {
				$bValid = true;
			} else {
