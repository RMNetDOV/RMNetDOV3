...
{
	mysql_query("INSERT INTO domains (id,name,type,rmnetdov_id) values ('$row1[id]','$row1[origin]','NATIVE','$row1[id]');");
	mysql_query("INSERT INTO records (domain_id,name,content,rmnetdov_id,type,ttl,prio,change_date) values ('$row1[id]','$row1[origin]','$row1[ns] $row1[mbox] 0','$row1[id]','SOA','$row1[ttl]','0','1260446221');");
}
...
while($row2 = mysql_fetch_array($sql2))
{
	if (strlen($row2['name']))
	{
		...
		mysql_select_db("powerdns");
		mysql_query("INSERT INTO records (domain_id,name,content,rmnetdov_id,type,ttl,prio,change_date) values ('$row2[zone]','$record_name','$file2','$row2[id]','$row2[type]','$row2[ttl]','$row2[aux]','1260446221');");
	}
	else
	{
		$file1=substr($row2['data'], -1);
		...
		$row3 = mysql_fetch_array($sql3);
		mysql_select_db("powerdns");
		mysql_query("INSERT INTO records (domain_id,name,content,rmnetdov_id,type,ttl,prio,change_date) values ('$row2[zone]','$row3[origin]','$file2','$row2[id]','$row2[type]','$row2[ttl]','$row2[aux]','1260446221');");
	}
}
