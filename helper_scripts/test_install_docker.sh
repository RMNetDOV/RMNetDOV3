...
if [ -f /usr/local/rmnetdov/interface/lib/config.inc.php ]; then
...
sed -i "s/^hostname=server1.example.com$/hostname=$HOSTNAME/g" /root/rmnetdov3_install/install/autoinstall.ini

service mysql start && php -q $CI_PROJECT_DIR/install/install.php --autoinstall=/root/rmnetdov3_install/install/autoinstall.ini
